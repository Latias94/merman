...
- Replaced Dugong's alpha.3 minimal `layout` and alternate `layout_dagreish` split with one transactional Mermaid-compatible pipeline. `dugong::layout` now returns non-exhaustive `LayoutError`, arithmetic failures no longer panic, and the non-Dagre `ranker = "none"` bypass is removed.
