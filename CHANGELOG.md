...
- Simplified `StateDiagramRenderModel::links` to one `StateDiagramRenderLink` per state, matching Mermaid's last-declaration-wins database semantics. `StateDiagramRenderLinks` is removed; `tooltip` remains the upstream-compatible string value.
