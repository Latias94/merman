...
- Resource-limit binding payloads now distinguish `ceiling` from `arithmetic_overflow` through the structured `details.resource.cause` field. Binding consumers must preserve the discriminator instead of classifying failures from display text.
- Reworked Rust analysis and editor ownership around sealed `AnalysisGeneration` values, explicit ready/rejected outcomes, policy-separated `AnalysisOptions`, parser-only facts schema 1, and cancellable caller-owned shared-source entry points. Final alpha.4 generations do not expose or retain a complete snapshot policy. See the [Rust and embedding API migration](docs/release/ALPHA3_TO_ALPHA4_UPGRADE_GUIDE.md#rust-and-embedding-api-migration) section for exact type, method, builder, and symbol-search replacements.
- Replaced `SourceMap::line_starts()` and `SourceMap::source_arc()` with behavioral line queries and `shared_source()`. Shared slices retain validated UTF-8 ranges over one `Arc<str>` and copy only through the explicit `to_owned_text()` boundary.
...
- Replaced Dugong's alpha.3 minimal `layout` and alternate `layout_dagreish` split with one transactional Mermaid-compatible pipeline. `dugong::layout`, low-level `rank`, and `network_simplex` now return non-exhaustive `LayoutError`; arithmetic and invalid Network Simplex tree states no longer panic, and the non-Dagre `ranker = "none"` bypass is removed.
- Updated graphlib and ELK public execution semantics to match the pinned Mermaid/Dagre source: canonical JavaScript array-index keys enumerate numerically, compound `GraphExecution` traces use source-backed postorder, and ELK/Dugong/Manatee work errors are non-exhaustive. Direct ELK importer callers must carry source `model_order` where edge order is observable and update exhaustive error matches.
...
- Finalized embedded LSP ownership around the ordered `MermanLspService`, a one-time `MermanClientSocket::split()`, and transport-owned scheduling. The final surface omits the interim `StdioService`, control-concurrency constant, and total-concurrency constant; the bundled stdio transport owns exact private control admission and reports unrecoverable saturation through the new exhaustive `StdioTermination::InputOverloaded` variant. Enable the `stdio` feature explicitly when building the bundled server, and send catalog/schema requests through the ordered service. #26
...
- Added semantic editor-theme presets, the `merman-modern` presentation profile, artifact-aware presentation discovery, and independent presentation-profile selectors in first-party CLI, Web, Playground, and Typst surfaces without coupling them to Mermaid theme selection or SVG cleanup.
...
- Calibrated the `interactive` layout-work ceiling to 800,000 owner-accounted units from the completed public benchmark-fixture corpus, leaving 102,248 units (about 14.66%) above `flowchart_large`; the stricter `constrained` profile remains unchanged.
...
- Restored sanitized Kanban ticket navigation in the Playground without admitting external rendering resources. Pointer input now starts as a pending tap and promotes to pan/pinch only after intent is clear; stale Compare publications disable navigation, and promoted link gestures suppress their synthetic click on desktop and mobile.
