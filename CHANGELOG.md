...
- Moved root-level CLI rendering flags behind explicit subcommands. During the `0.8.x` transition, commands such as `merman-cli -i diagram.mmd -o diagram.svg` are forwarded to the exact `merman-cli mmdc` parser and emit a deprecation warning unless `--quiet` is present. This root-level forwarding will be removed in `v0.9.0`; the explicit `merman-cli mmdc` compatibility subcommand remains supported. New workflows should use `merman-cli render` for one diagram or `merman-cli batch` for Markdown. The separation gives native and `mmdc@11.16.0` workflows independent parsers, defaults, validation, help, and publication guarantees instead of accepting format-specific no-ops. The compatibility path keeps upstream append-style default names such as `diagram.mmd.svg`, while native `render` replaces the source extension and writes `diagram.svg`.
