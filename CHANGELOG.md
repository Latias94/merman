...
- Broke the prerelease analysis facts wire contract to schema `2`: the generic parser/editor facts remain available, but the Flowchart-only rich graph field and its Rust/TypeScript/FFI projections are removed. Facts consumers must regenerate against schema `2`; diagnostics payload schema `1` is unchanged. No schema-1 facts decoder or deprecated alias remains.
