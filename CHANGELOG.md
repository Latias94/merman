...
- Moved advertised root-level CLI rendering flags behind explicit subcommands while permanently retaining commands such as `merman-cli -i diagram.mmd -o diagram.svg` as silent, unadvertised aliases for the exact `merman-cli mmdc` parser and execution path. New workflows should use `merman-cli render` for one diagram, `merman-cli batch` for Markdown, or explicit `merman-cli mmdc` when pinned compatibility is required. The separation gives native and `mmdc@11.16.0` workflows independent parsers, defaults, validation, help, and publication guarantees without breaking existing root-level scripts. The compatibility path keeps upstream append-style default names such as `diagram.mmd.svg`, while native `render` replaces the source extension and writes `diagram.svg`.
...
- Changed rich Rust analysis entry points to return explicit rejection outcomes: `Analyzer::analyze_generation`, `Analyzer::analyze_generation_shared`, and `analyze_document_generation{,_shared}` now return `AnalysisCaptureOutcome`, while editor analysis builders return `DocumentAnalysisOutcome`. Match `Ready` / `Rejected` or call `into_ready()`; cancellable variants still wrap the outcome in `Result<_, AnalysisCancelled>`, and `DocumentWorkspace::upsert` now returns `Result<DocumentSnapshot, AnalysisRejection>`.
- Removed the borrowed `Analyzer::analyze_generation_cancellable(&str, ...)` entry point. Cancellable rich capture is now `Analyzer::analyze_generation_shared_cancellable(Arc<str>, ...)`, which keeps full-source ownership promotion outside the cancellation boundary and reuses the caller's allocation. Non-cancellable shared callers can use `Analyzer::analyze_generation_shared`.
...
- Renamed the rich Rust ownership API without deprecated aliases: `AnalysisResult` is now `AnalysisGeneration`, `Analyzer::analyze_result` is `Analyzer::analyze_generation`, and `analyze_document_result{,_shared}` is `analyze_document_generation{,_shared}`. Replace old `payload()` or `diagnostics()` access with `generation.project(&policy)`, use `Analyzer::analyze()` for a diagnostics-only payload, and use `Analyzer::analyze_facts()` for the serialized facts contract. The TypeScript `AnalysisResult` name remains the public schema-1 diagnostic wire/API result and is intentionally unchanged.
- Changed `merman_editor_core::DocumentSnapshot` construction from caller-supplied URI and kind to `try_from_analysis_generation(version, generation)`. URI and document kind now come from the generation's canonical `SourceDescriptor`; generations without a source path return `DocumentSnapshotError::MissingSourcePath`.
...
- Changed custom `DiagramSemanticParser` overlays to accept `&ParseControl` and return `ParseControlResult<Result<Value>>`. Custom parsers must checkpoint long-running work, return cancellation through the outer result, and return Mermaid parse failures through the inner result. Exhaustive `merman_core::Error` matches must also handle the new `Error::ParseCancelled` variant used by non-cancellable facades when an overlay cancels unexpectedly; analysis reports that contract violation as the protected `merman.internal.parser_contract_violation` internal diagnostic instead of a Mermaid syntax error.
- Changed Analyzer entry points to honor the configured source kind directly: Markdown and MDX descriptors now use canonical fence extraction and `max_document_diagrams` admission instead of producing a whole-document Mermaid generation with a host-document identity.
- Changed `DiagnosticFix::edits` from mutable `Vec` storage to shared immutable `Arc<[DiagnosticFixEdit]>` storage. Construct fixes with `DiagnosticFix::new`; cloning or projecting a document-wide action now shares its edit allocation, while schema-1 JSON remains an ordinary repeated `edits` array and every owning diagnostic keeps its quick fix. Init-directive migration fixes now use cancellable frontmatter/YAML scans plus bounded config, frontmatter input/materialization, nesting, edit count, and replacement output; an oversized migration still emits its diagnostics without attaching the advisory fix. LSP code-action requests materialize a shared document fix only once even when several requested diagnostics own it.
...
- Changed `MermanClientSocket` into an explicit ownership token. Call `split()` once and drive the returned `MermanRequestStream` and `MermanResponseSink`; the unsplit socket no longer implements `Stream` or `Sink`, and dropping either half terminates the shared language session.
- Replaced the bundled stdio readiness handshake with explicit synchronous `StdioService::admit`, split the former handler limit into `LSP_ORDINARY_HANDLER_CONCURRENCY`, `LSP_CONTROL_HANDLER_CONCURRENCY`, and `LSP_TOTAL_HANDLER_CONCURRENCY`, and reserved an independently bounded control lane so cancellation and valid `exit` notifications remain reachable under ordinary saturation. Rename `StdioServer::concurrency_level` to `StdioServer::ordinary_concurrency_level`; `exit` requests are rejected and `shutdown` notifications are ignored without terminating the session.
- Removed direct `MermanLanguageServer::rule_catalog()` and `MermanLanguageServer::config_schema()` helpers. Send `RULE_CATALOG_METHOD` or `CONFIG_SCHEMA_METHOD` through the ordered service, or use `RuleCatalogResponse::current()` and `ConfigSchemaResponse::current()` when only the static Rust payload is needed.
...
- Replaced the incompatible prerelease binding-options grammar with Options JSON schema 2. Version 1 and unknown fields are rejected; generated Kotlin, Dart, Python, Swift/UniFFI, Web, and C helpers consume the shared schema authority, leave request-overlay profiles unset by default so constructor ceilings are inherited, and expose only overridable resource IDs for option construction.
- Moved direct UniFFI generic options into `MermanOperationRequest.options_json`; `execute(request)` no longer accepts a parallel options argument. One-shot requests may choose a runtime policy when constructing their temporary engine, while reusable request options deeply merge over an immutable baseline and cannot replace its constructor-owned policy.
- Kept the UniFFI projection stable across artifact profiles: lint catalogs remain callable without `analysis`, and host text-measurement types and reusable-engine entrypoints remain generated without `svg`; unavailable calls now return typed missing-capability errors instead of an empty catalog or a feature-dependent API shape.
...
- Removed the legacy `FlowchartElkBackend` selector and its CLI, binding JSON, and Web options without aliases. Flowchart ELK now always uses the source-backed Mermaid adapter and Eclipse ELK layered implementation; the older lightweight compatibility backend is removed.
...
- Changed the Typst transport resource contract to enforce the `constrained` policy for every call. Caller-provided `resources` profiles and numeric overrides are replaced at the plugin boundary instead of being allowed to select trusted or unbounded limits.
- Removed underscore and shorthand aliases for binding enum values. Use the documented kebab-case forms such as `resvg-safe`, `strip-existing-important`, `trusted-native`, `unbounded-for-trusted-input`, and the generated host-theme preset names; the Rust parser and Web types now expose the same closed value set.
...
- Reproduced Mermaid's locale-sensitive Swimlane ordering with fixed `en-US` ICU4X collation across native and WASM targets. Browser size budgets now record that semantic dependency explicitly; byte-order sorting remains intentionally unsupported because it changes layout behavior.
...
- Promoted the exact YAML stack from `granit-parser` and `serde-saphyr` 1.0.0 release candidates to their stable 1.0.0 releases while preserving the existing minimal, default-disabled feature selections.
...
- Added deterministic Bash, Elvish, Fish, PowerShell, and Zsh completion snapshots plus recursive command man pages to CLI release archives. Runtime `completion` output and tracked distribution assets are generated from the same feature-complete Clap command tree and checked for drift.
- Added explicit cargo-binstall archive mapping, a reusable Nix source package, and stable-only Scoop and WinGet submission candidates without claiming those external registries are already published. Version-specific cargo-dist installers now fail closed on SHA-256 verification; the final CLI/LSP bundle is checksum-bound, executed on every supported native target, reverified before publication, and covered by GitHub artifact attestations.
...
- Language-tooling crates now use the same invariant complete diagram catalog as every core build; `merman-lsp` reports the complete family capability set instead of a selected registry profile. #26
...
- Fixed the Web `initMerman({ wasm })` boundary to accept a raw URL, response, byte buffer, or compiled `WebAssembly.Module`; wasm-bindgen's `{ module_or_path }` envelope is now private to the wrapper, so downstream callers can remove that workaround and avoid its deprecated initialization warning.
...
- Preserved Mermaid's Architecture `seed: 0` opt-out semantics by consuming the operation-owned random stream continuously across FCoSE reruns; non-zero seeds alone restart the configured stream for each run.
