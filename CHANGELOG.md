...
- Removed root-level CLI rendering flags. Existing Merman commands such as `merman-cli -i diagram.mmd -o diagram.svg` must move to the pinned compatibility path, `merman-cli mmdc -i diagram.mmd -o diagram.svg`; new workflows should use `merman-cli render` for one diagram or `merman-cli batch` for Markdown. Removed root flags exit `2` with a migration diagnostic. This break gives native and `mmdc@11.16.0` workflows independent parsers, defaults, validation, help, and publication guarantees instead of accepting format-specific no-ops. The compatibility path keeps upstream append-style default names such as `diagram.mmd.svg`, while native `render` replaces the source extension and writes `diagram.svg`.
