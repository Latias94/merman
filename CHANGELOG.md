...
- Replaced the `merman-cli capabilities --json` document with CLI schema 2. Consumers should read `descriptor.digest` instead of `descriptor_digest`; the redundant `target: "native"` field is removed. The document now reports `cli_contract_version`, package and pinned compatibility versions, the compiled command set, and descriptor-backed capability/output objects so package-manager and artifact checks can validate the binary they actually installed.
