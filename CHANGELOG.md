...
- Raised the C ABI used by C, Android JNI, Apple Swift, and Flutter/Dart to version 3, and raised the UniFFI and WASM ABI versions to 3. Host text-measurement callback requests now include the required `phase` field; rebuild native artifacts, regenerate bindings where applicable, and update callbacks to handle the named measurement phases.
