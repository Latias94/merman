...
- Replaced the TextScan-capable `AnalysisFactsPayload` shape shipped in `0.8.0-alpha.3` with the sole parser-only facts v1 contract. `fact_source: "text_scan"` is removed, unavailable bodies use `"unavailable"`, and current writers include `rename_policy` on every semantic item. Readers conservatively decode an omitted additive policy as `"none"`; no legacy TextScan executor or dual facts path remains. The diagnostics-only `AnalysisPayload` independently remains v1. Facts schema versions are independent of native, UniFFI, and WASM ABI versions, LSP document revisions, and Mermaid `*-v2` ids.
