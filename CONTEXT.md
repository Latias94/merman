...
- Effective config and presentation theme should be projected into narrow views
  before diagram renderers consume them. Sequence, Class, and Flowchart are the
  first renderer-side family pilots for this: layout and SVG parity settings now
  flow through family-owned config views instead of scattered raw diagram
  namespace lookups.
