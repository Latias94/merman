...
the executable evidence described in `metadata/support.json`. Evidence kinds are restricted to
their package test runner's corpus, edit, query, binding, fuzz, schema, or metrics paths; header
recognition alone is not semantic validity and is not structured CST support.
The bootstrap verifier rejects every non-null tier until U2 replaces that lock with typed,
runner-produced gate receipts.
