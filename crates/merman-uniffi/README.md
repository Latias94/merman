...
> **Alpha:** the current source uses direct UniFFI binding API `8`; the immutable alpha.6 release uses API `6`. It is not the C ABI and not the text-measurement protocol; all three have independent version ownership. Regenerate bindings and ship them with the exact native library used to generate them.
...
API 7 adds optional `MermanDrawingListErrorDetails` to `MermanError.Binding`. Its `category`,
optional `family`, and optional `reason` preserve structured renderer-neutral export failures.
The extra field changes all binding error layouts, including errors without DrawingList details.
...
API 7 replaces `binding_api_version_v6()` with `binding_api_version_v7()` for the changed error
layout. The released API 6 projection otherwise passes every method checksum and fails while
decoding ordinary errors. The removed probe prevents old bindings from reaching that decoder.
Regenerate complete Swift/Python projections and replace their matching native library together.

The current API is 8. It replaces the probe with `binding_api_version_v8()` and adds optional
`MermanNormalLineMetrics` to the host callback result. Protocol-2 `normal-line-metrics` returns
line height and baseline offset atomically; older generated callback records are not compatible.
