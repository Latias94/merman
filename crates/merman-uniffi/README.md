...
> **Alpha:** the current source uses direct UniFFI binding API `7`; the immutable alpha.6 release uses API `6`. It is not the C ABI and not the text-measurement protocol; all three have independent version ownership. Regenerate bindings and ship them with the exact native library used to generate them.
