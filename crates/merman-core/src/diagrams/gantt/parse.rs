...
impl GanttAccDescrBlock {
    ...
    fn consume_remaining(mut self, cursor: &mut LineCursor<'_>) -> Self {
        while !self.complete {
...
fn construct_gantt_semantic_source(
    ...
    let result = parse_gantt_semantic_source_with_lexemes(code, meta, &mut lexemes);
