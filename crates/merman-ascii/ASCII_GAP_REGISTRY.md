...
| A-XY-010 | Richer XYChart layout policy after compact terminal legends. | `xychart::plot`, `xychart::render` | Plot-cell planning is split from row rendering through `xychart::plot`, and multi-series legend rows now render with stable fallback labels because the typed model does not preserve series titles. Remaining work is variable-size plot-area policy and any future model-backed series labels. | `cargo nextest run -p merman-ascii xychart` | README XYChart contract; 2026-06-24 legend closeout |
