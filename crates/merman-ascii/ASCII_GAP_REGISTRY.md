...
| A-GRAPH-010 | Flowchart subgraph direction overrides. | `graph::layout`, `graph::routing`, `graph::draw` | Shipped subset: canonical `LR` subgraphs inside canonical `TD` roots when routed edges stay entirely inside the subgraph. Remaining work is cross-boundary mixed-direction routing, broader local `TD` coverage, and nested mixed-direction behavior. | `cargo nextest run -p merman-ascii flowchart subgraph`; `cargo nextest run -p merman-ascii graph_fixture` | `FLOWCHART_SUPPORT.md`; 2026-06-01 local-direction subset |
