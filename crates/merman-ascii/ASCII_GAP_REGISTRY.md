...
| A-CLASSER-010 | Dense, cyclic, and more complex class/ER relation graph layouts. | `relation_graph`, `class::render`, `er::render` | Shared relation labels now cover multiline class/ER relationship labels across vertical stacks, same-endpoint lanes, and layered overlays. `LayeredRelationScene` owns layered box placement, lane draw ordering, route drawing, and direction-neutral spanning margins so cycle-closing reverse edges stay visible. Remaining work is topology depth: extend the layered planner or add another relation graph adapter; keep unresolved dense cases explicit where no deterministic text layout exists, and allow self-authored semantic fixtures when copied parity is the wrong oracle. | `cargo nextest run -p merman-ascii class er` | README shipped matrix; `ASCII_REFERENCE_COMPARISON.md`; class/ER tests |
