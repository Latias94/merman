...
| A-XY-010 | Richer XYChart layout policy after compact terminal legends. | `xychart::plot`, `xychart::render` | Plot-cell planning is split from row rendering through `xychart::plot`, multi-series legend rows now render with stable fallback labels because the typed model does not preserve series titles, and the compact plot area is configurable through `AsciiRenderOptions`. Remaining work is any future model-backed series labels and richer tick/data-label policies. | `cargo nextest run -p merman-ascii xychart` | README XYChart contract; 2026-06-24 plot-area closeout |
