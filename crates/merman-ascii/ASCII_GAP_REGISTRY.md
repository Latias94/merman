...
| A-XY-010 | Richer XYChart legends and scalable terminal plot area. | `xychart::plot`, `xychart::render` | Plot-cell planning is now split from row rendering through `xychart::plot`; remaining work is legend rows and variable-size plot-area policy. | `cargo nextest run -p merman-ascii xychart` | README XYChart contract; 2026-06-24 plot seam refactor |
