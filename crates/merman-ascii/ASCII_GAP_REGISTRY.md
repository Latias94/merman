...
| A-GRAPH-010 | Flowchart subgraph direction overrides. | `graph::layout`, `graph::routing`, `graph::draw` | Add local layout planning for `FlowSubgraph.dir`; define cross-boundary edge behavior. | `cargo nextest run -p merman-ascii flowchart subgraph`; `cargo nextest run -p merman-ascii graph_fixture` | `FLOWCHART_SUPPORT.md` |
...
| A-SEQ-030 | Sequence titles, actor shapes, and actor links/properties. | `sequence::model`, `sequence::validate`, `sequence::render` | Decide which Mermaid presentation metadata has useful terminal output; preserve explicit diagnostics for unsupported metadata. | `cargo nextest run -p merman-ascii sequence`; support-boundary tests | `SEQUENCE_SUPPORT.md` |
