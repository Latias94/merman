...
| A-CLASSER-010 | Dense, cyclic, and more complex class/ER relation graph layouts. | `relation_graph`, `class::render`, `er::render` | Shared relation labels now cover multiline class/ER relationship labels across vertical stacks, same-endpoint lanes, bidirectional same-pair lanes, and layered overlays. `LayeredRelationScene` owns layered box placement, lane draw ordering, route drawing, direction-aware label placement, and shared lane width budgeting so cycle-closing reverse edges and reverse parallel lanes stay visible. When layer reordering still cannot produce a readable deterministic relation drawing, or when the routed scene would exceed the configured grid budget, class/ER render explicit structured relation-summary fallback sections instead of failing; the shared summary adapter aligns source/connector/target columns and preserves Mermaid `<br>` labels as continuation rows without leaking markup. Remaining work is true topology depth: decide when a dense case deserves a routed grid rather than the honest summary adapter. | `cargo nextest run -p merman-ascii class er` | README shipped matrix; `ASCII_REFERENCE_COMPARISON.md`; class/ER tests |
