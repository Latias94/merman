...
| A-XY-010 | Richer XYChart layout policy after compact terminal legends. | `xychart::plot`, `xychart::render`; `merman-core::diagrams::xychart` | Plot-cell planning is split from row rendering through `xychart::plot`, multi-series legend rows use typed plot titles when present with stable fallbacks for untitled series, and the compact plot area is configurable through `AsciiRenderOptions`. Remaining work is richer tick/data-label policies. | `cargo nextest run -p merman-core xychart`; `cargo nextest run -p merman-ascii xychart` | README XYChart contract; 2026-06-24 series-title closeout |
