...
| Subgraphs | Supported subset | Titled group boxes, multiline title rows from explicit line breaks, nested groups, external nodes, and subgraph edge crossings covered by copied `mermaid-ascii` graph fixtures. |
...
| Subgraphs | Supported subset. | Titled, multiline-title, nested, and external-edge group layouts are covered by parser/model tests and copied `mermaid-ascii` graph fixtures. |
...
| Multiline subgraph labels | Ported | The title text can be represented, and group layout now reserves multiple centered title rows using the shared graph label splitter. | Covered by `flowchart_parser_multiline_subgraph_title_renders_centered_rows` and `render_flowchart_renders_model_multiline_subgraph_titles`. |
...
- `BT` and `RL` are root-direction transforms only. `FlowSubgraph.dir` overrides remain a separate
  layout follow-on.
- Multiline subgraph titles render explicit line breaks (`<br>`/escaped newline/model newline);
  automatic browser-style wrapping of long titles remains a separate follow-on.
