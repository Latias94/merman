...
#[tokio::test(flavor = "current_thread")]
async fn stale_workspace_symbol_contexts_return_workspace_error() {
    let store = test_store();
    let uri = Url::parse("file:///tmp/example.mmd").unwrap();
    ...
    let context =
        snapshot_context::snapshot_context_for_uri(&store, &uri, SnapshotContextKind::Structure)
            .await
            .expect("snapshot context build should not fail")
            .expect("expected snapshot context");
    {
    ...
    let error = snapshot_context::ensure_snapshot_contexts_current(
        &store,
        &[context],
        SnapshotContextKind::WorkspaceSymbols,
    )
    .await
    .expect_err("stale workspace symbol contexts should fail");

    assert_eq!(error.code, ErrorCode::ContentModified);
    assert!(error.message.contains("workspace symbol documents changed"));
}
