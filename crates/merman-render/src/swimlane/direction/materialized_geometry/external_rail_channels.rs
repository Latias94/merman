...
fn external_rail_for_segment(
    ...
    let (source, destination) = endpoint_rects_for(layout, &layout.original_edges[edge_index])?;
    if segment.vertical {
...
fn collect_external_rails(layout: &WorkingLayout) -> Vec<ExternalRail> {
    ...
    for (edge_index, edge) in layout.original_edges.iter().enumerate() {
        ...
        for segment in segments_for(&points) {
            if let Some(rail) = external_rail_for_segment(layout, edge_index, &points, &segment) {
                rails.push(rail);
            }
        }
    }
...
fn connected_components(rails: &[ExternalRail]) -> Vec<Vec<ExternalRail>> {
...
fn unique_coordinates_for(component: &[ExternalRail]) -> Vec<f64> {
...
fn visit_permutations(
    ...
    for (coordinate_index, coordinate) in coordinates.iter().copied().enumerate() {
        ...
        visit_permutations(coordinates, current, used, next, assignments);
        next.pop();
        used[coordinate_index] = false;
    }
}

fn coordinate_assignments_for(component: &[ExternalRail]) -> Vec<Vec<f64>> {
    let current: Vec<_> = component.iter().map(|rail| rail.coordinate).collect();
    let coordinates = unique_coordinates_for(component);
...
fn replacements_for_assignment(
    ...
    for (index, rail) in component.iter().enumerate() {
        let points = drafts
            .entry(rail.edge_index)
            .or_insert_with(|| rail.points.clone());
...
pub(in crate::swimlane::direction) fn reassign_crossing_external_rail_channels(
    ...
    for _ in 0..MAX_ITERATIONS {
        ...
        let mut best_bends = total_bends(&layout.original_edges, &empty);
        let mut best_displacement = f64::INFINITY;

        for component in connected_components(&collect_external_rails(layout)) {
            for assignment in coordinate_assignments_for(&component) {
                let Some(replacements) = replacements_for_assignment(&component, &assignment)
                else {
                    continue;
                };
                if !candidate_is_safe(layout, &replacements, &real_node_rects, &label_rects) {
                    continue;
                }
