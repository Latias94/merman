...
pub(in crate::svg::parity::flowchart::render::node) fn try_render_icon_circle(
    out: &mut String,
    ...
    // Port of Mermaid `iconCircle.ts` (`icon-shape default`). The nested icon SVG has an explicit
    // square viewport, so its browser `getBBox()` is the requested icon size on both axes.
    if let Some(icon_name) = common.node_icon.filter(|s| !s.trim().is_empty()) {
        ...
        let diameter = icon_size * std::f64::consts::SQRT_2 + FRAME_PADDING * 2.0;
