...
fn extract_clusters_recursively(
    ...
    fn extract_one_level(
        ...
        let mut external: std::collections::HashMap<String, bool> =
            std::collections::HashMap::new();
    ...
    let extracted_here = extract_one_level(graph, subgraphs_by_id);
    ...
    while let Some(mut frame) = stack.pop() {
        ...
        let children = extract_one_level(&mut frame.graph, subgraphs_by_id);
        frame.expanded = true;
        stack.push(frame);
...
fn layout_flowchart_v2_with_model(
    ...
    fn layout_graph_with_recursive_clusters(
        ...
        fn update_child_cluster_bounds(
            ...
            let ids = recursive_child_ids(graph, extracted);
            for id in ids {
                let Some(child) = extracted.get(&id) else {
                    continue;
                };
        ...
        while let Some(frame) = stack.pop() {
            if !frame.expanded {
                let Some((child_ids, parent_nodesep, parent_ranksep)) = (match &frame.cluster_id {
                    Some(id) => extracted.get(id).map(|g| {
                        (
                            recursive_child_ids(g, extracted),
                            g.graph().nodesep,
                            g.graph().ranksep,
                        )
                    }),
                    None => Some((
                        recursive_child_ids(graph, extracted),
                        graph.graph().nodesep,
                        graph.graph().ranksep,
                    )),
                }) else {
                ...
                for child_id in child_ids.iter().rev() {
                    ...
                    // clusters.
                    if let Some(child) = extracted.get_mut(child_id) {
            ...
            if let Some(cluster_id) = frame.cluster_id {
                let Some(mut current) = extracted.remove(&cluster_id) else {
                    continue;
                };
                update_child_cluster_bounds(
                    &mut current,
                    extracted,
                    subgraph_id_set,
                    title_total_margin,
                );
                inject_parent_cluster(&mut current, &cluster_id, cluster_node_labels);
                layout_one_graph(
                    &mut current,
                    extracted,
                    subgraph_id_set,
                ...
                extracted.insert(cluster_id, current);
            } else {
                update_child_cluster_bounds(graph, extracted, subgraph_id_set, title_total_margin);
                layout_one_graph(
                    graph,
                    extracted,
                    subgraph_id_set,
