...
fn linear_segments(points: &[crate::model::LayoutPoint]) -> Vec<PathSegment> {
...
fn basis_segments(points: &[crate::model::LayoutPoint]) -> Vec<PathSegment> {
    if points.is_empty() {
        return Vec::new();
    }
    if points.len() < 3 {
    ...
    let p = |index: usize| Point::new(points[index].x, points[index].y);
    let mut out = vec![PathSegment::MoveTo { to: p(0) }];
    ...
    let previous = p(points.len() - 2);
    let current = p(points.len() - 1);
    out.push(PathSegment::CubicTo {
        control1: Point::new(
            (2.0 * previous.x + current.x) / 3.0,
            (2.0 * previous.y + current.y) / 3.0,
