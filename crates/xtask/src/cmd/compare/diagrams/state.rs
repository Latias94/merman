...
pub(crate) fn compare_state_svgs(args: Vec<String>) -> Result<(), XtaskError> {
    ...
    let mut state = StateCompareState {
        root_deltas: Vec::new(),
    };
    ...
    run_svg_compare(
        ...
        |_, report, _paths, options| {
            let _ = writeln!(
                report,
                "# StateDiagram SVG Comparison\n\n- Upstream: `fixtures/upstream-svgs/state/*.svg` (pinned Mermaid baseline)\n- Local: `render_state_diagram_v2_svg` (Stage B)\n- Mode: `{}`\n- Decimals: `{}`\n",
                options.dom_mode, options.dom_decimals
            );
        },
        |_, stem, _| {
            crate::cmd::upstream_svg_compare_skip_reason("state", stem)
