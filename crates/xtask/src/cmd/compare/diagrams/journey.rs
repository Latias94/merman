...
use crate::cmd::compare::{
    ...
    sanitize_svg_id, svg_compare_engine, svg_compare_layout_opts, write_compare_result_section,
...
pub(crate) fn compare_journey_svgs(args: Vec<String>) -> Result<(), XtaskError> {
    ...
    let mut state = JourneyCompareState {
        root_deltas: Vec::new(),
    };
    ...
    run_svg_compare(
        ...
        |_, report, _paths, options| {
            let _ = writeln!(
                report,
                "# Journey SVG Comparison\n\n- Upstream: `fixtures/upstream-svgs/journey/*.svg` (pinned Mermaid baseline)\n- Local: `render_journey_diagram_svg` (Stage B)\n- Mode: `{}`\n- Decimals: `{}`\n",
        ...
        |state, input| {
            ...
            let merman_render::model::LayoutDiagram::JourneyDiagram(layout) = &layouted.layout
            else {
                return Err(format!(
