...
pub(crate) const DESCRIPTOR_PATH: &str = "contracts/editor-language/token-descriptor-v1.json";
...
pub(crate) fn descriptor_error(message: impl Into<String>) -> XtaskError {
...
pub(crate) fn read_descriptor(path: &Path) -> Result<TokenDescriptor, XtaskError> {
...
fn token_equivalence_artifact(
    ...
    let artifact = TokenEquivalenceArtifact {
        generated_by: "cargo run -p xtask -- gen-editor-language-contract",
...
pub(crate) fn apply_vscode_token_projection(
...
pub(crate) fn token_artifact_drift(
    root: &Path,
    descriptor: &TokenDescriptor,
) -> Result<Vec<PathBuf>, XtaskError> {
...
mod tests {
    ...
    fn vscode_manifest_projects_custom_tokens_scopes_and_enablement() {
        ...
        apply_vscode_token_projection(contributes, &descriptor).unwrap();
    ...
    #[test]
    fn token_artifacts_have_no_drift() {
        let root = crate::cmd::workspace_root();
        let descriptor = committed_descriptor();
        assert!(token_artifact_drift(&root, &descriptor).unwrap().is_empty());
    }
}
