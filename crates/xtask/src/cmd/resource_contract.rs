...
const FFI_RUST_OUTPUT: &str = "crates/merman-ffi/src/generated/resource_contract.rs";
...
fn generated_artifacts() -> Vec<(PathBuf, String)> {
    ...
    vec![
        (PathBuf::from(FFI_RUST_OUTPUT), render_c_rust()),
        (PathBuf::from(FFI_HEADER_OUTPUT), render_c_header()),
        (PathBuf::from(UNIFFI_RUST_OUTPUT), render_uniffi()),
        (PathBuf::from(WEB_OUTPUT), render_typescript()),
        (PathBuf::from(ANDROID_OUTPUT), render_kotlin()),
        (PathBuf::from(FLUTTER_OUTPUT), render_dart()),
