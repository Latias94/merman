...
pub(crate) fn flowchart_elk_svg_parity_stems() -> &'static [&'static str] {
    FLOWCHART_ELK_SVG_PARITY_STEMS
}
...
pub(crate) fn upstream_svg_baseline_skip_reason(
    ...
    if let Some(reason) = parser_only_fixture_reason(diagram, stem) {
        return Some(reason);
    }
...
pub(crate) fn parser_only_fixture_reason(
    diagram: &str,
    fixture_name_or_stem: &str,
) -> Option<&'static str> {
    merman_fixture_render_context::parser_only_fixture_reason(diagram, fixture_name_or_stem)
}
...
mod tests {
    ...
    #[test]
    fn flowchart_elk_svg_parity_registry_accepts_names_and_stems() {
    ...
    #[test]
    fn parser_only_exclusions_are_exact_source_backed_facts() {
        for (diagram, stem) in [
            (
                "flowchart",
            ...
            assert!(
                parser_only_fixture_reason(diagram, stem).is_some(),
                "missing explicit parser-only fact for {diagram}/{stem}"
            );
        }

        assert_eq!(
            parser_only_fixture_reason("flowchart", "new_parser_only_spec"),
            None,
            "a parser-only-looking filename must not create an exclusion"
        );
        assert_eq!(
            ...
            None,
            "the family is part of the exclusion identity"
        );
    }
