...
fn gen_upstream_svgs(args: Vec<String>) -> Result<(), XtaskError> {
    ...
    match diagram.as_str() {
        ...
        | "timeline" | "journey" => run_one(&workspace_root, &out_root, &mmdc, &diagram, filter),
        other => Err(XtaskError::UpstreamSvgFailed(format!(
            "unsupported diagram for upstream svg export: {other} (supported: er, flowchart, state, class, sequence, info, pie, packet, timeline, journey, all)"
        ))),
    }
}
