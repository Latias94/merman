...
fn gen_font_metrics(args: Vec<String>) -> Result<(), XtaskError> {
    ...
    const FLOWCHART_DIAGRAM_PADDING_PX: f64 = 8.0;
    const FLOWCHART_TITLE_DOMINANCE_THRESHOLD_PX: f64 = 80.0;
    ...
    let mut svg_title_bbox_samples: Vec<SvgTitleBboxSample> = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        // Collect flowchart titles (SVG `<text>`) to later derive a `svg_scale` factor that
        // approximates Mermaid's `getBBox()` viewport sizing.
        let Some(root_svg) = doc.descendants().find(|n| n.has_tag_name("svg")) else {
            continue;
        };
        let diagram_id = root_svg.attribute("id").unwrap_or_default();
        let Some(title_node) = doc.descendants().find(|n| {
            n.has_tag_name("text")
                && n.attribute("class")
                    .unwrap_or_default()
                    .split_whitespace()
        ...
        if matches!(backend.as_str(), "browser" | "puppeteer") {
            // We will populate `width_px` via `text.getBBox().width` in the browser after
            // collecting all samples (to avoid the "title-dominant" heuristic entirely).
            ...
            // Fallback path for non-browser backends: only keep "title-dominant" svgs where
            // `viewBox.width - 2*diagramPadding` approximately equals the title glyph bbox.
            let (viewbox_x, _viewbox_y, viewbox_w, _viewbox_h) = match parse_viewbox(root_svg) {
                Some(v) => v,
                None => continue,
            };
            if !(viewbox_w.is_finite() && viewbox_w > 0.0) {
                continue;
            }
            let Some(content_w) = estimate_flowchart_content_width_px(&doc) else {
                continue;
            };
            let bbox_w = viewbox_w - FLOWCHART_DIAGRAM_PADDING_PX * 2.0;
            if !(bbox_w.is_finite() && bbox_w > 0.0) {
                continue;
            }
            if bbox_w <= content_w + FLOWCHART_TITLE_DOMINANCE_THRESHOLD_PX {
                continue;
            }
            let title_chars = title_text.chars().count().max(1) as f64;
            let avg_em_per_char = bbox_w / title_font_size_px / title_chars;
            // Guardrail: if our "title-dominant" heuristic is wrong, the implied average per-character
            // advance becomes absurd and will poison the regression. Filter those cases out.
            if !(avg_em_per_char.is_finite() && (0.10..=1.20).contains(&avg_em_per_char)) {
                continue;
            }
            ...
            // Retain bbox extents and anchor position for asymmetric overhang estimation.
            let bbox_min_x = viewbox_x + FLOWCHART_DIAGRAM_PADDING_PX;
            let bbox_max_x = bbox_min_x + bbox_w;
            let anchor_x = title_node
                .attribute("x")
                .and_then(|v| v.parse::<f64>().ok())
                .unwrap_or(0.0);
            if bbox_min_x.is_finite()
                && bbox_max_x.is_finite()
                && anchor_x.is_finite()
                && bbox_max_x > bbox_min_x
            {
                svg_title_bbox_samples.push(SvgTitleBboxSample {
                    font_key: font_key.clone(),
                    text: svg_samples.last().unwrap().text.clone(),
                    font_size_px: title_font_size_px,
                    bbox_min_x,
                    bbox_max_x,
                    anchor_x,
...
(async () => {
  ...
  const widths = await page.evaluate(({ strings, fontFamily, fontSizePx }) => {
    ...
    for (const s of strings) {
      out.push(ctx.measureText(String(s)).width);
    }
    return out;
...
(async () => {
  ...
  const widths = await page.evaluate(({ strings, fontFamily, fontSizePx }) => {
    ...
    for (const s of strings) {
      ...
      t.setAttribute('x', '0');
      t.setAttribute('y', '0');
      t.setAttribute('style', `font-family:${ff};font-size:${fontSizePx}px;`);
      t.textContent = String(s);
      svg.appendChild(t);
      out.push(t.getBBox().width);
      svg.removeChild(t);
    }
    return out;
  }, { strings, fontFamily, fontSizePx });
...
"#;
    ...
    // Derive a simple SVG text scaling factor from title-dominant samples:
    // `svg_scale ≈ bbox(title_svg_text) / width(foreignObject_label_model)`.
    // This avoids overfitting per-character SVG glyph boxes while still matching Mermaid's
    // `getBBox()`-driven viewport sizes.
    ...
    for (font_key, ss) in svg_samples_by_font {
        ...
        const SVG_SCALE_BIAS: f64 = 1.0;
        if let Some(m) = median(&mut scales) {
            svg_scales_by_font.insert(font_key, (m * SVG_SCALE_BIAS).clamp(0.5, 2.0));
        }
    }

    // Derive first/last-character bbox overhangs (relative to the text-anchor position) from
    // title-dominant samples. This models the fact that SVG `getBBox()` can be slightly
    // asymmetric due to glyph overhangs and layout/rounding quirks.
    let mut svg_bbox_overhangs_by_font: BTreeMap<
        String,
        (f64, f64, Vec<(char, f64)>, Vec<(char, f64)>),
    > = BTreeMap::new();
    {
        let mut by_font: BTreeMap<String, Vec<SvgTitleBboxSample>> = BTreeMap::new();
        for s in svg_title_bbox_samples {
            by_font.entry(s.font_key.clone()).or_default().push(s);
        }

        for (font_key, ss) in by_font {
            let Some(html) = html_tables.get(&font_key) else {
                continue;
            };
            ...
            let mut left_by_char: BTreeMap<char, Vec<f64>> = BTreeMap::new();
            let mut right_by_char: BTreeMap<char, Vec<f64>> = BTreeMap::new();
            ...
            for s in ss {
                ...
                let font_size_px = s.font_size_px.max(1.0);
                let adv = line_width_px(
                    &html.entries,
                    html.default_em.max(0.1),
                    &html.kern_pairs,
                    text,
                ...
                let bbox_min_x = s.bbox_min_x;
                let bbox_max_x = s.bbox_max_x;
                let anchor_x = s.anchor_x;
                if !(bbox_min_x.is_finite()
                    && bbox_max_x.is_finite()
                    && anchor_x.is_finite()
                    && bbox_max_x > bbox_min_x)
                ...
                let left_extent = anchor_x - bbox_min_x;
                let right_extent = bbox_max_x - anchor_x;
                if !(left_extent.is_finite() && right_extent.is_finite()) {
                    continue;
                }

                let half = adv / 2.0;
                let denom = (font_size_px * scale).max(1.0);
                let left_em = (left_extent - half) / denom;
                let right_em = (right_extent - half) / denom;
                if !(left_em.is_finite() && right_em.is_finite()) {
                    continue;
                }

                left_by_char.entry(first).or_default().push(left_em);
                right_by_char.entry(last).or_default().push(right_em);
            ...
            for (ch, mut vs) in left_by_char {
                if let Some(m) = median(&mut vs) {
                    let v = m.clamp(-0.2, 0.2);
            ...
            for (ch, mut vs) in right_by_char {
                if let Some(m) = median(&mut vs) {
                    let v = m.clamp(-0.2, 0.2);
