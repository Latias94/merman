...
fn gen_upstream_svgs(args: Vec<String>) -> Result<(), XtaskError> {
    ...
    match diagram.as_str() {
        ...
        | "radar" | "quadrantchart" | "treemap" | "xychart" | "mindmap" => {
            run_one(&workspace_root, &out_root, &mmdc, &diagram, filter)
        }
        other => Err(XtaskError::UpstreamSvgFailed(format!(
            "unsupported diagram for upstream svg export: {other} (supported: er, flowchart, gantt, mindmap, state, class, sequence, info, pie, requirement, packet, timeline, journey, kanban, gitgraph, quadrantchart, c4, block, radar, treemap, xychart, all)"
        ))),
    }
}
...
fn check_upstream_svgs(args: Vec<String>) -> Result<(), XtaskError> {
    ...
    match diagram.as_str() {
        ...
        "er" | "flowchart" | "state" | "class" | "sequence" | "info" | "pie" | "requirement"
        | "packet" | "timeline" | "journey" | "kanban" | "gitgraph" | "gantt" | "c4" | "block"
        | "radar" | "quadrantchart" | "treemap" | "mindmap" => check_one(
            &workspace_root,
            &baseline_root,
            &out_root,
            diagram.as_str(),
        ...
        other => Err(XtaskError::UpstreamSvgFailed(format!(
            "unsupported diagram for upstream svg check: {other} (supported: er, flowchart, gantt, mindmap, state, class, sequence, info, pie, requirement, packet, timeline, journey, kanban, gitgraph, quadrantchart, c4, block, radar, treemap, all)"
        ))),
    }
}
