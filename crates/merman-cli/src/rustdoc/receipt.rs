...
pub(crate) fn check(
    ...
    let generated = super::generate(config, resources, stderr)?;
    super::document::verify_input_snapshots(config, &generated, resources)?;
...
mod tests {
    ...
    fn expected(root: &Path) -> ExpectedRustdocBundle {
        ...
        .unwrap();
        let config =
            super::super::config::load(&root.join("merman-rustdoc.toml"), &resources()).unwrap();
        let generated = super::super::generate(&config, &resources(), &stderr()).unwrap();
    ...
    fn generated_receipt_obeys_the_same_limit_as_receipt_acquisition() {
        ...
        .unwrap();
        let config =
            super::super::config::load(&root.path().join("merman-rustdoc.toml"), &resources())
                .unwrap();
        let generated = super::super::generate(&config, &resources(), &stderr()).unwrap();
