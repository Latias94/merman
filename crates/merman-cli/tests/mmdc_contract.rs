...
#[test]
fn mmdc_quadrant_pipeline_preserves_parity_color_and_materializes_resvg_fallback() {
    ...
    let safe_svg = String::from_utf8(resvg_safe.stdout).expect("resvg-safe stdout should be utf8");
    assert!(
        parity_svg.contains(r#"fill="hsl(240, 100%, NaN%)" stroke="hsl(240, 100%, NaN%)""#),
        "default SVG output should preserve the pinned Mermaid point color:\n{parity_svg}"
    );
    assert!(
