...
| Browser editor intelligence | Closed | Local Monaco uses a dedicated `@mermanjs/web/editor` module Worker backed by `editor-language`, the 35-family Rust catalog, native ABI 3, diagnostics schema 1, and facts schema 1. TypeScript syntax heuristics are not a fallback path. |
