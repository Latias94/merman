...
| ARCH-003 | Closed | `crates/merman-core/src/family.rs` is the single built-in catalog and projects detector order, aliases, semantic/editor/render adapters, metadata, config namespaces, and authoring headers. Every core build uses that same language catalog. |
...
| Browser editor intelligence | Closed | Local Monaco uses a dedicated `@mermanjs/web/editor` module Worker backed by `editor-language`, the 35-family Rust catalog, native ABI 2, diagnostics schema 1, and facts schema 2. TypeScript syntax heuristics are not a fallback path. |
