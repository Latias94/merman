...
The `with_vendored_text_measurer()` call above is evidence for Zed's pinned prerelease API, not the
current Merman contract. Current integrations use the built-in deterministic fallback or install a
host measurer through the operation environment/service boundary.
...
| 2026-07-24 | [zed#61586](https://github.com/zed-industries/zed/issues/61586) (open issue) | Host font-rasterization boundary | Merman can use a font-agnostic deterministic estimate or host-provided measurement, but GPUI/usvg selects the actual glyph fallback used for pixels. Zed must make its Mermaid font database and fallback selection cover the document's scripts. |
...
| zed-industries/zed#56914 / #51466 / #51623 / #56695 | Fonts missing or substituted incorrectly in Zed/GPUI rasterization | Mostly host integration boundary | `merman` can use deterministic fallback measurement or a host measurement callback. Actual glyph rasterization and font fallback are handled by the host SVG/raster stack. |
