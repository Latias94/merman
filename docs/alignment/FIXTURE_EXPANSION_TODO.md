...
- 2026-02-27: Upstream examples fixtures import (16 fixtures from `repo-ref/mermaid/packages/examples/src/examples/*.ts`, with upstream SVG baselines) via `xtask import-upstream-examples --with-baselines`. (6 examples were skipped as duplicates of existing fixtures.)
...
- 2026-02-17: Timeline authored “common stress” fixtures (6 fixtures under `fixtures/timeline/timeline_stress_common_*`, with upstream SVG baselines). Added a fixture-derived Timeline root viewport override to keep the global `parity-root` gate green.
- 2026-02-17: Kanban authored “common stress” fixtures (12 fixtures under `fixtures/kanban/kanban_stress_common_*`, with upstream SVG baselines). Added a fixture-derived Kanban root viewport override to keep the global `parity-root` gate green.
...
- 2026-02-17: Flowchart authored stress fixtures batch (12 fixtures under `fixtures/flowchart/stress_flowchart_icons_*_051..062.mmd`, with upstream SVG baselines) focusing on FontAwesome tokens in node/edge labels, strict-mode click sanitization, and icon labels inside clusters/subgraphs. Added fixture-derived Flowchart root viewport overrides for the new fixture IDs to keep the global `parity-root` gate green.
...
- 2026-02-17: Architecture authored stress fixtures batch (12 fixtures under `fixtures/architecture/stress_architecture_batch6_*_085..096.mmd`, with upstream SVG baselines). Added fixture-derived Architecture root viewport overrides for the new fixture IDs and extended Architecture SVG parity forced-wrap cases for edge/service/group titles to keep the global `parity-root` gate green.
...
- 2026-02-17: Class authored stress fixtures batch (12 fixtures under `fixtures/class/stress_class_*_013..024.mmd`, with upstream SVG baselines). Added fixture-derived Class root viewport overrides and hardened Class SVG parity for namespace rendering when the root viewBox indicates the `-8px` x-margin behavior (render namespaces as nested subgraphs with an outer placeholder `<g class="clusters"/>`, matching Mermaid DOM) plus strict-mode link sanitization (`about:blank` produces `<a>` without `xlink:href`), keeping the global `parity-root` gate green.
...
- 2026-02-16: Gantt Cypress rendering fixtures import (12 fixtures from `repo-ref/mermaid/cypress/integration/rendering/gantt.spec.js`, with upstream SVG baselines) using the now-retired broad Cypress importer. Hardened Gantt exclude-layer `transform-origin` parity for timezone-shifted date-only inputs, keeping the global `parity-root` gate green.
- 2026-02-16: Packet Cypress rendering fixtures import (5 fixtures from `repo-ref/mermaid/cypress/integration/rendering/packet.spec.ts`) using the now-retired broad Cypress importer. Kept the global `parity-root` gate green.
- 2026-02-16: GitGraph Cypress rendering fixtures import (30 fixtures from `repo-ref/mermaid/cypress/integration/rendering/gitGraph.spec.js`) using the now-retired broad Cypress importer. Added fixture-derived GitGraph root viewport overrides (29 entries) to keep the global `parity-root` gate green.
- 2026-02-16: Journey Cypress rendering fixtures import (9 fixtures from `repo-ref/mermaid/cypress/integration/rendering/journey.spec.js`) using the now-retired broad Cypress importer. One fixture includes an upstream-invalid task score (`size: 2rem`) which Mermaid renders as `NaN`; `merman` preserves this in DOM parity mode via a `scoreIsNaN` semantic marker and Journey root viewport overrides to keep the global `parity-root` gate green.
...
- 2026-02-16: Class Cypress rendering fixtures incremental import (8 fixtures from `repo-ref/mermaid/cypress/integration/rendering/classDiagram*.spec.{js,ts}`) using the now-retired broad Cypress importer. Refreshed Class root viewport overrides for the new fixture IDs and aligned Class note-label XHTML output with Mermaid's DOMPurify pipeline (so safe HTML like `<a target=\"_blank\">` becomes real DOM nodes and gets `rel=\"noopener\"`), keeping the global `parity-root` gate green.
...
- 2026-02-15: Class Cypress rendering fixtures batch import (25 fixtures from `repo-ref/mermaid/cypress/integration/rendering/classdiagram*.spec.{js,ts}`, with upstream SVG baselines) using the now-retired broad Cypress importer. Hardened Class SVG DOM parity for Markdown `<em>/<strong>` runs in HTML labels and namespace wrapper structure; refreshed Class root viewport overrides as needed to keep the global `parity-root` gate green.
...
- 2026-02-14: Architecture Cypress rendering fixtures import (8 fixtures from `repo-ref/mermaid/cypress/integration/rendering/architecture.spec.ts`, with upstream SVG baselines). Normalized legacy architecture edge shorthands (`a L--R b`, `a (L--R) b`, `a L-[Label]-R b`) into Mermaid@11.12.3 CLI-compatible Langium grammar (`a:L -- R:b`, `a:L -[Label]- R:b`) during Cypress import to keep `gen-upstream-svgs --diagram architecture` seeded baselines working and the global `parity-root` gate green.
...
- 2026-02-14: Class HTML demo fixtures import (10 fixtures from `repo-ref/mermaid/demos/classchart.html` via `<pre class="mermaid">`, with upstream SVG baselines). (1 additional demo block deferred because Mermaid CLI `@11.12.3` fails to parse it; the block contains the line `class People List~List~Person~~`.)
...
- 2026-03-05: Imported the previously deferred Flowchart Cypress fixture `upstream_cypress_flowchart_v2_spec_should_be_possible_to_use_syntax_to_add_labels_with_trail_spaces_067` (trail spaces + edge/link) with upstream SVG baselines, keeping the global `parity-root` gate green.
