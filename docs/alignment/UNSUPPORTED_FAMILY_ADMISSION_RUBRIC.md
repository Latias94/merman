...
| 5 | `railroad-beta`, `railroad-ebnf-beta`, `railroad-abnf-beta`, `railroad-peg-beta` | `packages/mermaid/src/diagrams/railroad` | Port the renderer around the shared AST and add representative fixtures for all four dialects. | New in the pinned 11.16 baseline. The upstream family has separate detector/parser specs for base, EBNF, ABNF, and PEG variants. Detector/header/config/admission visibility and parser/editor facts exist locally; layout, renderer, fixtures, baselines, and compare tooling are not admitted yet. |
