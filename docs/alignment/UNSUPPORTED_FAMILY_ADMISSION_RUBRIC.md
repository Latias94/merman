...
| 4 | `swimlane-beta` header / `swimlane` id | `packages/mermaid/src/diagrams/swimlanes` | Port the swimlane layout/render path on top of Flowchart semantics before SVG admission. | New in the pinned 11.16 baseline. Detector/header/config/admission visibility exists locally, and the parser/editor-facts path reuses Flowchart as upstream does. Swimlane layout, renderer admission, fixtures, baselines, and compare tooling are not admitted yet. |
| 5 | `railroad-beta`, `railroad-ebnf-beta`, `railroad-abnf-beta`, `railroad-peg-beta` | `packages/mermaid/src/diagrams/railroad` | Start with source-test ports for the four parser variants and an LSP-friendly AST; evaluate a grammar generator only if spans and recovery stay first-class. | New in the pinned 11.16 baseline. The upstream family has separate detector/parser specs for base, EBNF, ABNF, and PEG variants. Detector/header/config/admission visibility exists locally; parser/layout/render support is not admitted yet. |
| 6 | `cynefin-beta` | `packages/mermaid/src/diagrams/cynefin` | Build a semantic model and renderer plan from source fixtures before adding it to the primary matrix. | New in the pinned 11.16 baseline. Detector/header/config/admission visibility exists locally, but parser, semantic model, layout, renderer, fixtures, baselines, and compare tooling are not admitted yet. |
