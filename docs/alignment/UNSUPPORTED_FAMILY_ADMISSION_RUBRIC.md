...
| 4 | `wardley-beta` | `packages/mermaid/src/diagrams/wardley` | Defer behind smaller families unless Wardley maps become a direct requirement. | Parser and builder are feature-rich, renderer is roughly 1k lines, and the family carries many map-specific concepts: axes, stages, pipelines, links, trends, annotations, notes, accelerators, deaccelerators, and sourcing strategy overlays. Feasible headlessly, but large. |
