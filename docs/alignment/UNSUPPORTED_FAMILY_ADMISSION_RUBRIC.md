...
| 6 | `cynefin-beta` | `packages/mermaid/src/diagrams/cynefin` | Build a renderer plan from source fixtures before adding it to the primary matrix. | New in the pinned 11.16 baseline. Detector/header/config/admission visibility and parser/editor facts exist locally, but layout, renderer, fixtures, baselines, and compare tooling are not admitted yet. |
