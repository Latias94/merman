...
| 2 | `ishikawa` / `ishikawa-beta` | `packages/mermaid/src/diagrams/ishikawa` | Decide rough/handDrawn and main-matrix admission policy. | Simple Jison grammar and tree DB. Renderer is larger and uses SVG `getBBox()` plus optional rough output, but it is still a pure headless geometry problem that fits existing text/root-bounds seams. Phase 2 now has upstream SVG baselines, config/theme coverage, `compare-ishikawa-svgs`, and a green family-local DOM parity gate; rough mode remains deferred. |
