...
| 4 | `venn-beta` | `packages/mermaid/src/diagrams/venn` | Audit or port the pinned `@upsetjs/venn.js@2.0.0` layout adapter before implementation; see `VENN_BETA_ADMISSION_PLAN.md`. | Parser/DB are manageable, but upstream rendering delegates circle layout to `@upsetjs/venn.js`, then uses D3 DOM and optional rough output. Do not fake this with a local circle layout unless the algorithm is audited or ported. |
