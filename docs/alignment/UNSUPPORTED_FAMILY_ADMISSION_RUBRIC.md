...
|---:|---|---|---|---|
| 1 | `treeView-beta` header / `treeView` id | `packages/mermaid/src/diagrams/treeView` | Open a small family workstream when new-family work is approved. | Smallest renderer and DB surface. Parser delegates to `@mermaid-js/parser`, DB builds a simple indentation tree, renderer is recursive SVG lines/text with deterministic dimensions. It lacks diagram-local upstream specs, so start from syntax docs and generated SVG baselines. |
| 2 | `ishikawa` / `ishikawa-beta` | `packages/mermaid/src/diagrams/ishikawa` | Open after `treeView` or if fishbone diagrams become product-critical. | Simple Jison grammar and tree DB. Renderer is larger and uses SVG `getBBox()` plus optional rough output, but it is still a pure headless geometry problem that fits existing text/root-bounds seams. Port `ishikawa.spec.ts` before renderer work. |
