...
|---:|---|---|---|---|
| 1 | `treeView-beta` header / `treeView` id | `packages/mermaid/src/diagrams/treeView` | Decide main-matrix admission after family-local DOM parity evidence. | Smallest renderer and DB surface. Parser delegates to `@mermaid-js/parser`, DB builds a simple indentation tree, renderer is recursive SVG lines/text with deterministic dimensions. Phase 2 now has upstream SVG baselines, `compare-tree-view-svgs`, and a green family-local DOM parity gate. |
| 2 | `ishikawa` / `ishikawa-beta` | `packages/mermaid/src/diagrams/ishikawa` | Expand theme/config/rough policy coverage before main-matrix admission. | Simple Jison grammar and tree DB. Renderer is larger and uses SVG `getBBox()` plus optional rough output, but it is still a pure headless geometry problem that fits existing text/root-bounds seams. Phase 2 now has upstream SVG baselines, `compare-ishikawa-svgs`, and a green family-local DOM parity gate; rough mode remains deferred. |
| 3 | `eventmodeling` | `packages/mermaid/src/diagrams/eventmodeling` | Decide `entity`, `note`, and `gwt` policy before main-matrix admission. | Renderer is modest, but DB/state construction is large and calculates swimlanes, relations, wrapped HTML/code labels, and text dimensions. Phase 2 now has upstream SVG baselines, `compare-eventmodeling-svgs`, and a green family-local DOM parity gate; explicit unsupported-statement policy is still needed. |
