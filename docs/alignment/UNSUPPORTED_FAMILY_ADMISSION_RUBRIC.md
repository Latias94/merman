...
| 4 | `venn-beta` | `packages/mermaid/src/diagrams/venn` | Port the pinned `@upsetjs/venn.js@2.0.0` layout kernel plus minimal `fmin` helpers before renderer DOM work; see `VENN_BETA_ADMISSION_PLAN.md`. | Parser/DB are manageable, but upstream rendering delegates circle layout, intersections, text centres, and SVG paths to `@upsetjs/venn.js`, then uses D3 DOM and optional rough output. The audit rejects approximate circle layout and keeps JS as an oracle only. |
