...
| 5 | `railroad-beta`, `railroad-ebnf-beta`, `railroad-abnf-beta`, `railroad-peg-beta` | `packages/mermaid/src/diagrams/railroad` | Port the renderer around the shared AST and then add layout/SVG baselines for all four dialects. | New in the pinned 11.16 baseline. The upstream family has separate detector/parser specs for base, EBNF, ABNF, and PEG variants. Detector/header/config/admission visibility, parser/editor facts, unified AST JSON, and semantic fixtures exist locally; layout, renderer, SVG baselines, and compare tooling are not admitted yet. |
