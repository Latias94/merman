...
| 4 | `swimlane-beta` header / `swimlane` id | `packages/mermaid/src/diagrams/swimlanes` | Design a parser/LSP strategy before admitting; decide whether it should share Flowchart semantic infrastructure or stay a separate typed family. | New in the pinned 11.16 baseline. Detector/header/config/admission visibility exists locally, but parser, semantic model, layout, renderer, fixtures, baselines, and compare tooling are not admitted yet. |
