...
The JSON document uses capability schema 2 and current CLI contract 5. It includes the Merman package version, pinned Mermaid and `mmdc` versions, compiled command/capability/output sets, and the canonical descriptor digest. ASCII-enabled builds also expose schema-1 family/profile/encoding/fallback discovery plus success/error report schemas and streams. The CLI contract version tracks native command behavior independently from this pinned compatibility baseline. An `mmdc` behavior change requires a Mermaid baseline alignment, updated tests, this register, and a migration note. Merman does not silently follow the latest npm release.
