...
| RaTeX math | Local CLI enables `math` by default; upstream uses browser Mermaid/KaTeX behavior. | Rust CLI should render math without extra feature friction. | Keep `--math-renderer none|ratex`; no-default build still rejects. |
...
- [x] Enable `math` and ASCII/Unicode output by default for `merman-cli`.
