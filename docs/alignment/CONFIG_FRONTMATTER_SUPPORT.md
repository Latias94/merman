...
| `securityLevel` | Yes | Yes | Yes | Partial by family | Core sanitizer and URL formatting consume the root field (`format_url_matches_mermaid_utils_spec`, sanitize tests, and family parser tests for flowchart/class/state/gantt links). `flowchart_svg_security_level_controls_unsafe_click_href_rendering` proves Flowchart SVG omits unsafe strict-mode hrefs while preserving loose-mode custom protocols, matching Mermaid `formatUrl` and Flowchart link rendering semantics. Class, State, and Gantt have parser/model evidence; add focused SVG tests before claiming rendered parity for every link path. Mermaid's browser iframe behavior for `sandbox` is outside the headless SVG contract. |
...
- `securityLevel` has shared sanitizer/model coverage and focused Flowchart SVG link coverage.
  Browser-specific `sandbox` iframe isolation is not represented by standalone headless SVG output,
  so family SVG tests should focus on observable link/target/sanitization DOM.
