...
| `layout` | Yes | Yes | Partial | Flowchart + Class ELK source-backed lanes | Detector/family selection paths preserve flowchart layout side effects; `parse_metadata_with_type_sync_preserves_flowchart_elk_layout_side_effect`, `full_build_detects_flowchart_elk_and_sets_layout`, `render_svg_returns_svg_for_flowchart_elk`, and `headless_renderer_renders_flowchart_elk_svg`. Flowchart `layout: elk` now uses the source-backed Mermaid ELK adapter / Eclipse ELK layered port by default. Class `layout: elk` selects the feature-gated Class ELK adapter and reuses the Class SVG renderer; `render_model_dispatch_uses_elk_for_class_layout_config` and `class_svg_elk_layout_preserves_existing_renderer_semantics` cover the dispatch and rendered SVG semantics. The dedicated Flowchart ELK probe lane covers every upstream `flowchart-elk.spec.js` exact render call and all 57 unique layout bodies. |
...
- Flowchart `layout: elk` is renderable through the source-backed ELK lane and defaults to that
  ...
  feature-gated through the existing ELK adapter path and reuse the Class SVG renderer so class
  styles, links, notes, namespaces, relation labels, and cardinality terminals stay on the same
  semantic renderer contract. Mermaid's current public Flowchart ELK config surface is
