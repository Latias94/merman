...
- Merman's typed default site policy intentionally extends the upstream six-key `secure` value with
  CSS-bearing fields such as `themeVariables`, `themeCSS`, and fonts. Custom site lists are enforced
  recursively when producing `effective_config`; trusted parity harnesses can explicitly select the
  upstream six-key behavior.
