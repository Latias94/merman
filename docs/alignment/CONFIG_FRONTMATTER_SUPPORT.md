...
| `layout` | Yes | Yes | Partial | Flowchart ELK source-backed lane | Detector/family selection paths preserve flowchart layout side effects; `parse_metadata_with_type_sync_preserves_flowchart_elk_layout_side_effect`, `full_build_detects_flowchart_elk_and_sets_layout`, `render_svg_returns_svg_for_flowchart_elk`, and `headless_renderer_renders_flowchart_elk_svg`. Flowchart `layout: elk` now uses the source-backed Mermaid ELK adapter / Eclipse ELK layered port by default. The dedicated ELK probe lane covers every unique upstream `flowchart-elk.spec.js` layout body; six remaining exact calls are duplicate-body fixture gaps. |
| `flowchart.defaultRenderer` | Yes | Yes | Detector/family selection + Flowchart ELK source-backed lane | Flowchart ELK source-backed lane | `crates/merman-core/src/detect/mod.rs`, `crates/merman-core/src/family.rs`, flowchart detector tests, flowchart-elk metadata tests, Flowchart ELK render smoke tests, and `xtask check-flowchart-elk-source-backed-probes`. `elk` selection is preserved and renderable through the source-backed ELK path; broad main-matrix admission remains separate from the dedicated ELK probe gate. |
...
- Flowchart `layout: elk` is renderable through the source-backed ELK lane and defaults to that
  lane for public render entry points. The dedicated ELK probe gate covers every unique upstream
  `flowchart-elk.spec.js` layout body; broad main-matrix admission is still separate, and six exact
  upstream calls remain duplicate-body fixture gaps rather than unique layout gaps.
