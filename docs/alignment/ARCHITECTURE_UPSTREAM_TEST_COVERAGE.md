# Architecture Upstream Test Coverage (Mermaid@11.16.0)
...
## Mermaid 11.16 additions

Source-backed 11.16 deltas:

- `align row ...` and `align column ...` parser directives are persisted as `layoutHints`
  (`repo-ref/mermaid/packages/mermaid/src/diagrams/architecture/architectureDb.ts`)
- Architecture rendering merges user-declared layout hints into FCoSE alignment constraints and
  emits matching relative placement constraints
  (`repo-ref/mermaid/packages/mermaid/src/diagrams/architecture/architectureRenderer.ts`)
- declared alignments take precedence over heuristic spatial-map alignment groups to avoid
  conflicting FCoSE constraints
  (`repo-ref/mermaid/packages/mermaid/src/diagrams/architecture/architectureRenderer.ts`)

Coverage in `merman`:

- parser/model tests: `crates/merman-core/src/diagrams/architecture.rs`
  (`architecture_align_layout_hints_are_parsed`,
  `architecture_align_rejects_unknown_member_with_exact_span`,
  `architecture_align_rejects_duplicate_member_with_exact_span`)
- LSP/editor facts test: `crates/merman-core/src/diagrams/architecture.rs`
  (`architecture_align_editor_facts_preserve_spans`)
- render constraint-plan test: `crates/merman-render/src/architecture.rs`
  (`architecture_fcose_input_plan_applies_layout_hints`)
...
The Cypress suite mixes shorthand syntax (which Mermaid CLI `@11.12.3` renders as an error SVG)
and newer Langium syntax. For end-to-end CLI baselines + DOM parity, we keep the raw Cypress strings as
`*_parser_only_` fixtures and add normalized variants that use the Mermaid@11.12.3 grammar:
