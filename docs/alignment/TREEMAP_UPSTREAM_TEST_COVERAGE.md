# Treemap Upstream Test Coverage (Mermaid@11.16.0)
...
- treemap SVG parity compare via `cargo run -p xtask -- compare-treemap-svgs --check-dom --dom-mode parity --dom-decimals 3` (skips only exact family-scoped parser-only facts)

Scope: Mermaid tag `@11.16.0`.

Source: `repo-ref/mermaid/cypress/integration/rendering/treemap/treemap.spec.ts`
...
- invalid bare `classDef` token behavior (upstream CLI renders an error diagram): `fixtures/treemap/upstream_treemap_classdef_and_css_compiled_styles_db.mmd`

Docs-derived fixtures (rendered via official Mermaid CLI, version-pinned):

Source: `repo-ref/mermaid/docs/syntax/treemap.md`
