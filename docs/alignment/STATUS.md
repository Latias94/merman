...
| `swimlane-beta` header / `swimlane` id | Present at `packages/mermaid/src/diagrams/swimlanes` | Detector/header/config/admission visibility exists. Flowchart parser and editor-facts reuse exists. No swimlane layout, renderer admission, fixtures, or upstream SVG baselines yet. | Not admitted to the SVG matrix; layout/render admission is tracked by the 11.16 parity workstream. |
...
| `railroad-beta`, `railroad-ebnf-beta`, `railroad-abnf-beta`, `railroad-peg-beta` | Present at `packages/mermaid/src/diagrams/railroad` | Detector/header/config/admission visibility exists. All four variants have parser/editor facts and unified AST JSON. Renderer, fixtures, and upstream SVG baselines are not admitted yet. | Not admitted; layout/render admission is tracked by the 11.16 parity workstream. |
| `cynefin-beta` | Present at `packages/mermaid/src/diagrams/cynefin` | Detector/header/config/admission visibility exists. Parser/editor facts model domains, items, transitions, common title/accessibility fields, and self-loop filtering. No local layout, renderer, fixtures, or upstream SVG baselines. | Not admitted; layout/render admission is tracked by the 11.16 parity workstream. |
