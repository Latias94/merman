...
| Evidence audit | Inventory and corpus counts checked against the worktree on 2026-07-17 |
| Admission | 35 families in the primary SVG matrix; `zenuml` is compatibility-only |
| Root viewport | Active convergence: ER, Mindmap, and State still have open `parity-root` work |
| Current worktree gate | Pending refresh; no green global gate is claimed. |
...
| --- | --- | --- |
| `zenuml` | Compatibility-only | Sequence-model subset; its upstream renderer is browser-only. |
