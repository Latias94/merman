...
| `railroad-beta`, `railroad-ebnf-beta`, `railroad-abnf-beta`, `railroad-peg-beta` | Present at `packages/mermaid/src/diagrams/railroad` | Parse-only semantic fixture coverage exists for all four variants. Parser/editor facts produce unified AST JSON for IR, EBNF, ABNF, and PEG. Renderer, layout snapshots, and upstream SVG baselines are not admitted yet. | Parse-only admitted; layout/SVG admission is staged behind the shared railroad renderer port. |
