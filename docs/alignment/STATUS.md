...
|---|---|---|---|
| `treeView-beta` header / `treeView` id | Present at `packages/mermaid/src/diagrams/treeView` | Phase 2 admission infrastructure exists: detector, typed parser/model, layout, Stage B SVG renderer, semantic/layout fixtures, upstream SVG baselines, and `compare-tree-view-svgs`. DOM parity still has wrapper class residuals. | Keep outside the main matrix until family-local DOM parity is green or residuals are formally accepted; see `TREEVIEW_MINIMUM.md` and `PHASE2_PARITY_BACKLOG.md`. |
| `ishikawa` / `ishikawa-beta` | Present at `packages/mermaid/src/diagrams/ishikawa` | Phase 2 admission infrastructure exists: detector, typed parser/model, layout, Stage B SVG renderer, semantic/layout fixtures, upstream SVG baselines, and `compare-ishikawa-svgs`. DOM parity still has root width residuals. | Keep outside the main matrix until family-local DOM parity is green or residuals are formally accepted; see `ISHIKAWA_MINIMUM.md` and `PHASE2_PARITY_BACKLOG.md`. |
| `eventmodeling` | Present at `packages/mermaid/src/diagrams/eventmodeling` | Phase 2 admission infrastructure exists: detector, typed parser/model, layout, Stage B SVG renderer, semantic/layout fixtures, upstream SVG baselines, and `compare-eventmodeling-svgs`. DOM parity still has root width residuals. | Keep outside the main matrix until family-local DOM parity is green or residuals are formally accepted; see `EVENTMODELING_MINIMUM.md` and `PHASE2_PARITY_BACKLOG.md`. |
