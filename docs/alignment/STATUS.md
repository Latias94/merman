...
Recent progress: flowchart Markdown measurement in HTML-like mode now ignores `<strong>/<em>` width deltas for mixed
styling, but still applies bold deltas for fully-bold labels (e.g. `**Two**`), matching upstream foreignObject bbox
sizing more closely (fixes `upstream_markdown_strings` in strict SVG XML parity).
