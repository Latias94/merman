...
The closure was re-measured on 2026-08-15 at revision
...
measurements. Even so, reducing 214 normal packages to 142 does not turn an in-process renderer
...
| `merman-rustdoc` | 0.8.0-alpha.5, 2026-08-09 | 242 / 242 | Attribute proc macro | Compile time | None | 142 normal packages minimum; 214 default |
...
## Exact CLI distribution Rustdoc product-path measurement

This measurement closes SC5 independently of the disposable U7 renderer loop. It exercised the
actual `merman-cli rustdoc build/check` product path from the exact `cli-release` feature recipe,
including Markdown scanning, dual-theme rendering, SVG admission and rebasing, receipt creation,
transactional publication for `build`, and full expected-bundle comparison for `check`.

**Verdict:** **PASS.** The 1, 10, and 100 unique-diagram workloads all completed within the default
resource policy. Total generated bytes grew from 31,442 to 3,048,260 while peak build RSS grew from
25.547 MiB to 32.938 MiB and peak check RSS grew from 25.391 MiB to 35.703 MiB. These three bounded
points are not an asymptotic proof, but they show no unbounded or output-proportional retained-memory
growth across the required workload range.

### Product measurement identity

The release binary was built on 2026-08-15 from branch `refactor/rustdoc-cli-generation` at
revision `f55baa19a6833c11483b80ced11bb812c8230833`. The tree contained the in-review Rustdoc theme,
test, and distribution-asset fixes; the SHA-256 of that pre-measurement diff was
`b3831c7282ffec3e2b062b9ae21db1269f0fe985f68e7ee45ff8e0a6401a4293`. The measurement itself did
not modify tracked files.

| Identity | Recorded value |
| --- | --- |
| Host | Mac16,7, Apple M4 Pro, arm64, 14 logical CPUs, 51,539,607,552 bytes physical memory |
| OS | macOS 26.5.1 build 25F80; Darwin 25.5.0 |
| Rust | `rustc 1.95.0 (59807616e 2026-04-14)`, LLVM 22.1.2 |
| Cargo | `cargo 1.95.0 (f2d3ce0bd 2026-03-21)` |
| `Cargo.lock` SHA-256 | `ee22330d16c7bc79ad45121be6fe0c8bc2e789150372b4d7390a83d282ded7e2` |
| Feature recipe | `analysis,ascii,icons,jpeg,layout-cytoscape,layout-elk,markdown,math,network-icons,parallel-markdown,pdf,png,rustdoc,shell-completions,svg,system-clock,system-random,system-timezone,system-timing` |
| Release binary | 42,199,168 bytes; SHA-256 `ad199a5bec82ae8ac0d8de6b9bfcadd7da6e54be6f589b8879311db6db659950` |
| Runtime policy | Default resource profile; Rustdoc rendering remains sequential under KTD17; `LC_ALL=C`, `TZ=UTC` |

The exact release build was serialized and excluded from workload timings:

```bash
features="$(python3 scripts/artifact_profile_recipe.py cli-release)"
CARGO_BUILD_JOBS=1 cargo build --release --locked -p merman-cli \
  --no-default-features --features "$features"
```

Each sample used a fresh temporary package root with one `bench` fragment and 1, 10, or 100 unique
`flowchart LR` fences. Every diagram used distinct node identifiers and labels, so invocation-local
source deduplication could not collapse the workload. Each `build` sample started without a managed
output root; its immediately following `check` sample read the successfully published fragment and
receipt. Cargo was not running during these samples. macOS `/usr/bin/time -l` reported process wall
time and maximum resident set size; its wall-time display has 0.01-second resolution.

### Raw samples and summary

| Diagrams | Generated fragment + receipt | Build `real`, raw | Build median | Build peak RSS | Check `real`, raw | Check median | Check peak RSS |
| ---: | ---: | --- | ---: | ---: | --- | ---: | ---: |
| 1 | 31,442 B | 0.14; 0.13; 0.14 s | 0.14 s | 26,787,840 B (25.547 MiB) | 0.01; 0.01; 0.01 s | 0.01 s | 26,624,000 B (25.391 MiB) |
| 10 | 306,140 B | 0.15; 0.18; 0.16 s | 0.16 s | 27,656,192 B (26.375 MiB) | 0.03; 0.03; 0.03 s | 0.03 s | 27,639,808 B (26.359 MiB) |
| 100 | 3,048,260 B | 0.37; 0.36; 0.36 s | 0.36 s | 34,537,472 B (32.938 MiB) | 0.22; 0.23; 0.23 s | 0.23 s | 37,437,440 B (35.703 MiB) |

The full maximum-resident-set-size samples in bytes were:

```text
build/1:   26492928 26640384 26787840
build/10:  27426816 27557888 27656192
build/100: 34439168 34537472 34308096
check/1:   26624000 26378240 26427392
check/10:  27639808 27328512 27213824
check/100: 37126144 37339136 37437440
```

The timed command shape was identical for every fresh fixture:

```bash
/usr/bin/time -l target/release/merman-cli rustdoc build \
  --config "$FIXTURE/merman-rustdoc.toml" --quiet
/usr/bin/time -l target/release/merman-cli rustdoc check \
  --config "$FIXTURE/merman-rustdoc.toml" --quiet
```
