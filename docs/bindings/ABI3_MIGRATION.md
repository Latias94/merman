...
3. Set `MermanNativeApi.struct_size` to the host table capacity. On success, Merman writes the common prefix and reports the producer's full table size in that field.
...
Every record except `MermanNativeApi` requires exactly the generated `struct_size`. The API table alone interprets `struct_size` as caller capacity and returns producer size.
