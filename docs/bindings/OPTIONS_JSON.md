...
The runtime catalog also reports seven native-export hard caps: `max_svg_conversion_isolation_depth`, `max_svg_conversion_filter_primitives_per_filter`, `max_total_svg_conversion_filter_primitives`, `max_svg_conversion_subroots`, `max_nested_svg_images`, `svg_backend_tree_nodes`, and `svg_backend_tree_depth`. They describe fixed recursion and structure guards owned by the export backend. They remain finite under `unbounded-for-trusted-input` and cannot be supplied in `resources.limits`; generated builders reject attempts to override them.
