...
| Kanban | Partial | Structured text | Columns, stable group/card ids, assignments, metadata, and deterministic `Unassigned` grouping. Authored labels and metadata use quoted byte-length framing. | Drag/drop and board presentation are not represented; duplicate or empty ids are rejected. |
