...
- `7e68646` — `perf(manatee): cut small-graph repulsion overhead`
  - Reduces COSE-Bilkent repulsion fixed-cost by reusing per-pair derived values and inlining the
    non-overlap force computation in the hot O(n^2) loop.
