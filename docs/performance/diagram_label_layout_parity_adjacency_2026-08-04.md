...
Preregistered on 2026-08-04 before inspecting any timing result from candidate
`4feb702879249f739709ea45c669075daf349cd1`.
...
- Candidate: `4feb702879249f739709ea45c669075daf349cd1`
- Toolchain: Rust `1.95.0` (`x86_64-pc-windows-msvc`)
- Base worktree: `E:\Rust\merman-perf-label-base`
- Candidate worktree: `E:\Rust\merman-perf-label-fixed`
...
python tools/bench/compare_self.py `
  ...
  --head-dir E:\Rust\merman-perf-label-fixed `
  --base-label 1d571a007-label-parity-base `
  --head-label 4feb70287-ordered-adjacency `
  --base-package merman `
  --head-package merman `
  --base-bench pipeline `
  --head-bench pipeline `
  ...
  --head-target-dir E:\Rust\merman\target\performance\label-parity-adjacency-fixed `
...
ten nodes per layer and `14 * (N / 10 - 1)` directed edges. The existing full public Dugong layout
benchmark runs in balanced `base A, candidate A, candidate B, base B` order. Each revision shares
one build target and saves each Criterion sample under a distinct immutable baseline name.

Build-only preparation:

```powershell
$env:CARGO_TARGET_DIR='E:\Rust\merman\target\performance\label-parity-adjacency-base-dugong'
cargo +1.95.0 bench --locked -p dugong --bench layout --no-run

$env:CARGO_TARGET_DIR='E:\Rust\merman\target\performance\label-parity-adjacency-fixed-dugong'
cargo +1.95.0 bench --locked -p dugong --bench layout --no-run
```

Measured ABBA sequence, using Criterion 0.8 defaults:

```powershell
$env:CARGO_TARGET_DIR='E:\Rust\merman\target\performance\label-parity-adjacency-base-dugong'
cargo +1.95.0 bench --locked -p dugong --bench layout -- 'dugong_layout/plan' --save-baseline base-a

$env:CARGO_TARGET_DIR='E:\Rust\merman\target\performance\label-parity-adjacency-fixed-dugong'
cargo +1.95.0 bench --locked -p dugong --bench layout -- 'dugong_layout/plan' --save-baseline candidate-a
cargo +1.95.0 bench --locked -p dugong --bench layout -- 'dugong_layout/plan' --save-baseline candidate-b

$env:CARGO_TARGET_DIR='E:\Rust\merman\target\performance\label-parity-adjacency-base-dugong'
cargo +1.95.0 bench --locked -p dugong --bench layout -- 'dugong_layout/plan' --save-baseline base-b
```
...
node count, the two within-revision medians differ by more than 15 percent.
...
`ln(N)`. The structural curve passes only when the mean candidate slope exceeds the mean base slope
by no more than `0.20` and neither `N=800` nor `N=1600` regresses by both more than 10 percent and
more than 50 microseconds. Correctness and stable-identity gates remain mandatory.
