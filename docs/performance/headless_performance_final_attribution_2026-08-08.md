...
| Analysis diagnostics | `DiagnosticsOnly` avoids retaining rich indexes and Flowchart facts, but diagnostic-bearing Flowchart projection still runs. No current allocation or public-operation timing counter isolates that projection, so its reachable saving remains unqualified. | `merman-analysis` projection over shared parser evidence. | Rejected-not-admitted for deeper specialization. | Keep the narrow retained-object split; do not add a parser capture policy without complete diagnostics and recovery evidence. |
