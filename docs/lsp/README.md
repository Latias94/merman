...
- Mermaid-backed compatibility rules such as
  ...
  when the pinned Mermaid source or docs expose the same warning. The former covers
  diagram-specific `htmlLabels` settings such as `flowchart.htmlLabels` and `class.htmlLabels`.
  They do not imply quickfix support unless the diagnostic carries explicit `DiagnosticFix`
  metadata.
