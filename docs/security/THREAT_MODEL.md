...
| Diagram config downgrades `securityLevel` or injects CSS through config | The seven-key default site policy protects host authority and arbitrary stylesheets. A shared CSS-value boundary admits source typography/theme values for both init and frontmatter, rejecting declaration, resource, and entity injection before theme calculation. The upstream artifact remains six-key data. | Core config, parse metadata, and public render API tests. |
