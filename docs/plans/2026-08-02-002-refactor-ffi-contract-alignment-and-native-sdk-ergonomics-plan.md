...
- **Authority:** `abi/merman-v3.json` owns the C wire contract. `capabilities/feature-surface-v1.json` owns operation and capability identity. `crates/merman-bindings-core` owns shared request, result, error, service, options, and runtime-catalog semantics. Generated projections and transport-owned tests prove each language surface. `v0.8.0-alpha.3` is the released comparison point; dependency and artifact-weight checks evaluate the current descriptor-owned products instead of freezing an untagged internal tree.
- **Execution profile:** Fearless coordinated pre-release refactor. Rust and language source APIs may break. Delete closed or misleading abstractions instead of preserving compatibility shims. Keep native ABI 3's current wire layout and size-tagged memory-safety rules; historical partial-table consumers are not a supported SDK target.
...
The remaining work is not another transport rewrite. It is a final contract-shaping pass before alpha.4. The C ABI needs one descriptor-owned contract and stronger misuse resistance without a parallel semantic approval graph. The Rust facade needs constructor-owned services and future-proof public data types. Language wrappers need open runtime vocabularies, accurate metadata discovery, complete named operations, and result APIs that do not hide resource-driven output plans. Native hosts also need Mermaid-compatible Iconify packs without importing CLI, filesystem, network, or async dependencies.
...
- R1. Native ABI 3 uses `abi/merman-v3.json` as the sole wire and semantic authority for numeric codes, records, function slots, ownership, and lifecycle behavior. Size-tagged discovery remains a caller-memory safety boundary, while historical partial-table headers are not compatibility fixtures. Descriptor-owned unsafe-caller preconditions must state that every provided record/buffer is suitably aligned or deliberately read unaligned, fully readable, alive, and immutable for the call, with no concurrent mutation. Runtime validation returns typed errors only for detectable shape/range/overlap faults; dangling or unreadable pointers remain C caller violations that no ABI can safely probe.
- R2. CI must validate the descriptor schema and resolved operation relationships, regenerate Rust/C/Dart projections deterministically, and prove the current generated header through compile-run consumer tests. The descriptor-derived minimum-prefix and full-descriptor digests are output metadata, not separately hand-approved baselines. Breaking pre-release changes update the descriptor and its generated consumers directly; no current-full semantic snapshot or monotonic snapshot verifier is maintained.
...
- R5. Every native status with a specialized error kind must serialize the matching kind. The current generated-header consumer must exercise discovery, catalog collection, engine creation, execution, result release, close retry behavior, and successful quiescent close; reduced-capacity discovery canaries prove caller-capacity writes remain bounded. Historical partial-table headers are not compatibility fixtures.
...
- R10. Native SDKs must accept standard IconifyJSON collections, whether complete packs or host-curated subsets, through a bounded builder that publishes immutable reusable pack state. High-level SDKs expose one immutable pack value (`json` plus optional registration-name/prefix override) and one transactional factory with no public mutation methods. Android and Flutter name their host snapshots `MermanIconPackSet`; Rust and UniFFI retain registry terminology for parsed native service state. C, JNI, and Flutter constructors borrow pack buffers only for the constructor call and return only after the engine owns the parsed state. Merman performs no acquisition or slicing; full collections are supported when they fit the published limits, and hosts must pre-trim larger collections.
...
- R26. Verify the current descriptor-owned artifact profiles directly with a small, reviewable set of required-package, forbidden-package, and forbidden-feature claims. Do not freeze complete dependency tuples or maintain a parallel dependency lock/provenance system: `Cargo.lock`, cargo-deny, RustSec policy, and legal verification own dependency identity and provenance. Semantic-only builds must not gain SVG/icon dependencies; SVG builds must not pull acquisition/network/async dependencies solely for icon registries; UniFFI bindgen remains opt-in; native packages retain descriptor-owned recipes. Native-profile denylists reject CLI/tooling, acquisition/network/async, parallel-markdown, and production bindgen packages while preserving documented renderer/export residuals. Artifact size and clean-build timing are machine-sensitive measurements collected only when making a specific performance claim, not merge contracts.
...
- AE2. Given the current generated header and library, a C consumer discovers the complete release table, creates an engine, executes semantic JSON, frees every result, retries a busy close, and closes successfully. A reduced-capacity canary proves discovery never writes a partial field or crosses caller memory.
...
- AE11. Given any row in the shared 13-operation matrix, the descriptor and bindings-core projection define the expected operation ID, media type, metadata schema, URI rule, and capability-gated failure. Platform owners consume representative rows through their real package smoke; the full matrix is exercised once by the authoritative core/Node contract rather than copied into every transport.
...
- AE15. Given the current wrapper and release-matched producer, decoding validates all advertised schema-1 sections and actual table capacity controls service availability. Non-empty services require the service-aware constructor and are never ignored or silently downgraded.
- AE16. Given the current descriptor-owned artifact recipes, the verifier fails if a semantic-only profile gains `merman-render` or if a native binding gains CLI, acquisition, network, async, parallel, or production-bindgen dependencies. Artifact size and clean-build timing are measured only for a focused performance claim, not required as machine-sensitive merge approvals.
...
- ABI 3 descriptor validation, generated projections, and current-header consumer tests pass from one authority.
...
- All canonical operations are covered by one shared 13-operation matrix plus compiled or executed representative language usage examples. A package smoke must prove construction, one real render, and release/close; it must not duplicate the complete matrix when another owner already executes it.
- Current native profiles satisfy their user-visible dependency boundaries without a parallel lockfile or machine-sensitive merge budget.
...
- KTD1. Keep `abi/merman-v3.json` as the sole native ABI authority and append the service constructor after `metadata_collect`. Generated projections and current-header consumer tests prove the selected contract; historical partial headers and separately frozen semantic snapshots are deleted. Pre-release breaking changes modify the descriptor directly instead of synchronizing an approval graph. Governs R1-R5 and R10-R16.
...
- **Dependency and artifact weight:** All new Rust implementation lives behind the existing `svg` capability and reuses dependencies already compiled by `merman-render`. Semantic-only and analysis-only profiles must not gain renderer dependencies, and native bindings must not gain CLI, acquisition, network, async, parallel, or production-bindgen packages. Artifact size is measured only when supporting a focused performance claim.
...
1. Establish descriptor-owned dependency boundary claims without capturing a second lockfile or machine-sensitive size baseline.
2. Consolidate and harden ABI semantics in the descriptor, then establish binding-core request/result/service, validated artifact-surface, metadata-registry, and shared-operation-matrix foundations.
...
| Broad generated changes hide dependency growth. | Native packages gain unintended runtime weight through new packages or heavier features/build roles. | Verify current descriptor-owned required/forbidden dependency claims; collect artifact-size evidence only for a focused performance investigation. |
...
- **Files:** `abi/merman-v3.json`, `abi/README.md`, `crates/xtask/src/cmd/native_abi.rs`, `crates/merman-ffi/src/lib.rs`, `crates/merman-ffi/tests/c_consumer_smoke.c`, and native ABI documentation.
- **Approach:** Move opaque scalar, handle, and unsafe caller-memory preconditions into descriptor-owned data and generate all language projections from that descriptor. Introduce sign-bit-preserving, low-bit domain-tagged token issuance for engine and result registries, documenting that tokens prevent accidental cross-kind use but do not authorize untrusted tenants. Enforce the documented zero-initialized `out_engine` precondition before allocation, centralize status-kind construction, reject the `NONE` sentinel separately from unknown codes, and use the current generated header as the only consumer fixture. Reduced-capacity canaries prove discovery remains memory-safe without promising historical partial-table compatibility.
- **Test scenarios:** Reject invalid descriptor shapes and unresolved operation relations; prove deterministic generated-file freshness; supply a nonzero `out_engine`; exhaust engine tokens and result-allocation tokens independently; on result-token exhaustion assert no result/token leak and that the engine/service graph remains usable; collide engine and result counters; verify every generated signed-64 projection remains positive; pass a token to the wrong API; execute `NONE`; execute an unknown code; serialize busy and reentrant failures; discover with exact, truncated, and appended table capacities; run the current consumer through create/execute/free/busy-close/success-close.
...
- **Verification:** Renderer and binding-core icon tests pass with `svg`, including pinned-source sanitizer differentials and the calibration report. Current native profiles satisfy R26's required/forbidden dependency boundaries, and non-SVG profiles do not gain renderer dependencies.
- **Dependencies:** U2.
...
- **Verification:** Generated C/Rust/Dart artifacts are fresh, C and C++ header smoke tests pass, the current consumer passes, and reduced-capacity discovery stays within caller memory.
- **Dependencies:** U1-U3.
...
- **Approach:** Rename the one-shot/discovery facade to `Merman` and the reusable type to `MermanEngine`, deleting `MermanReusableEngine`. Expose one direct `MermanEngine(options, services)`-shaped constructor; delete the facade factory, callback-specialized constructor, and combinatorial overloads. Generate an immutable `MermanIconPack`-shaped value with UTF-8 JSON and optional registration-name/prefix override, plus a transactional reusable icon-pack factory; Android and Flutter name their host snapshot `MermanIconPackSet`, while parsed native registries retain registry terminology. Services accept the immutable pack state and optional text measurer. Add generic metadata, typed operation metadata/output plans, result-returning binary methods, named analysis-facts/SVG-plan methods, and preserve simple conveniences. Rust/UniFFI share registries; JNI/Flutter borrow packs during construction. Extend each transport exposure only after its constructor exists. Generate Kotlin and Dart transport-exposure projections from the bindings-core registry, and make their runtime-catalog validators enforce both required payload schemas and any present option/service sections without handwritten transport vocabularies. Non-empty services require the service-aware constructor and are never silently ignored. Retryable UniFFI close detaches under synchronization and destroys after unlock; busy/reentrant outcomes move nothing. High-level close is idempotent, so among concurrent close calls one detaches/drops and later observers succeed without a second drop; C stale-token behavior remains unchanged. Regenerate Swift/Python and add package decoders where needed.
- **Test scenarios:** Generated UniFFI/Swift/Python surfaces expose exactly one reusable-engine constructor and one immutable icon factory; each public Swift/Python/Kotlin/Dart README golden path compiles or runs for one-shot, reusable optional services, metadata/result inspection, and deterministic close; package owners run one representative operation smoke while the authoritative shared matrix remains centralized; named helper parity; bytes equal result bytes; limited and unknown future output plans; metadata dispatch; complete/full and curated icon packs; registry reuse; borrowed buffers released; callback/service conflicts; constructor performs no callback; callback-engine cycle; busy/reentrant retry; close-vs-admission linearization; two concurrent high-level closes are idempotent with one drop; destructor re-entry has no deadlock; post-close methods fail; token/publication exhaustion rolls back; unknown runtime IDs; exports/checksums.
...
- **Approach:** Validate dependencies from the current descriptor-owned recipes instead of checking in a second dependency graph. Document migrations and unchanged wire versions, including Iconify complete-pack/subset expectations, fixed limits, sanitization/browser boundary, and callback destruction. Verify U2-U8 deletions left no stale enums/names/constructors/dispatchers/icon mutators/XML fallback/dead projections. Refresh artifacts through sole owners. Keep dependency claims limited to user-visible boundaries and run legal, advisory, size, and performance owners independently. Keep one full native SKU. Final reporting has separate public-native and private-Node readiness rows; both must be green to complete this goal, but a Node-only failure is not misreported as a public-native API regression.
- **Test scenarios:** Stale generated/docs/API names or unsafe claims; alpha.3 migration; artifact recipe drift; semantic-only profile gains a renderer; a profile gains a forbidden acquisition/runtime/bindgen dependency; package export omission; release workflow misses generated input; public-native green/Node red and inverse status reporting remain truthful.
...
- Dependency verification checks the current descriptor-owned profiles and deliberately has no
  immutable closure snapshot.
- Native binding profiles share explicit denylists for CLI/tooling, acquisition/network/async,
  parallel-markdown, and production bindgen packages.
- Artifact size and clean-build timing are measured only for explicit performance work; they are
  not stable cross-machine merge gates.
...
- `cargo nextest run --locked -p merman-bindings-core --no-default-features --features 'svg,analysis,ascii,png,jpeg,pdf,layout-cytoscape,layout-elk,math,native-runtime'`
- `cargo nextest run --locked -p merman-render --no-default-features`
- `cargo nextest run --locked -p merman-ffi --no-default-features`
- `cargo nextest run --locked -p merman-ffi -p merman --no-default-features --features 'merman-ffi/svg,merman-ffi/analysis,merman-ffi/ascii,merman-ffi/png,merman-ffi/jpeg,merman-ffi/pdf,merman-ffi/layout-cytoscape,merman-ffi/layout-elk,merman-ffi/math,merman-ffi/native-runtime,merman/complete-svg'`
- `cargo nextest run --locked -p merman-uniffi --no-default-features`
- `cargo nextest run --locked -p merman-uniffi -p merman --no-default-features --features 'merman-uniffi/svg,merman-uniffi/analysis,merman-uniffi/ascii,merman-uniffi/png,merman-uniffi/jpeg,merman-uniffi/pdf,merman-uniffi/layout-cytoscape,merman-uniffi/layout-elk,merman-uniffi/math,merman-uniffi/native-runtime,merman/complete-svg'`
- `cargo nextest run --manifest-path crates/merman-node/Cargo.toml --locked --no-default-features --features 'svg,layout-cytoscape,layout-elk,math,transport-napi,merman-bindings-core/png,merman-bindings-core/native-runtime' --lib --cargo-quiet`
...
- SVG native profiles keep icon support within their selected rendering closure and do not pull acquisition/network/async packages solely because icon registries became public.
- Artifact size and clean-build/link timing are non-blocking evidence captured only for a performance claim; stale checked-in reports and machine-specific ceilings are never treated as proof about the current tree.
- `merman-uniffi` includes bindgen and Cargo metadata only with `binding-generation`.
...
- The descriptor and generated-artifact freshness checks own the current ABI surface; output handles must start at zero, both engine and result token exhaustion are tested, and the current consumer completes create/execute/free/close lifecycle paths.
...
- One bindings-core-owned 13-operation matrix drives the authoritative semantic parity check. Swift, Python, Kotlin, Dart, and Node package-owned smokes compile or execute a representative construction/render/release path and rely on the shared projection for exhaustive operation vocabulary coverage.
...
- Current descriptor-owned artifact profiles satisfy their required/forbidden dependency claims, and public-native and private-Node readiness are reported separately.
- All generated files, examples, docs, changelogs, package exports, dependency policies, and legal projections match the final code.
...
- **U1:** Descriptor validation and generated-current-header tests prove the selected ABI, output-engine preconditions and both token-exhaustion paths are covered, reduced-capacity discovery cannot cross caller memory, token domains reject cross-kind misuse, and `NONE` is distinct from unknown operations.
...
- **U4:** New C records and the single service-aware constructor slot are append-only, fully generated, handle-free, pointer-preconditioned, misuse-resistant, and exposed through the current release table.
- **U5:** Every runtime catalog exactly describes callable metadata, accepted option groups, providers, and constructor services; future IDs round-trip without dispatch, and WASM/Typst call sites use the same contract without product expansion.
...
- **U7:** UniFFI, Apple, Python, Android, and Flutter expose the `Merman`/`MermanEngine` model, immutable icon factories, composed services, complete operations, generic metadata, typed plans, compiled golden examples, and deterministic idempotent callback-safe lifecycles with lock-free foreign destruction.
...
- **U9:** Documentation, migration, generation, dependency, legal, package, workflow, and release projections are current; current-tree dependency claims are consumed; public-native and private-Node readiness are independently green.
...
- **ABI 4 for every fix:** rejected because the selected pre-release ABI 3 descriptor can express the service additions directly; generated consumers, not a parallel semantic freeze file, prove the result.
