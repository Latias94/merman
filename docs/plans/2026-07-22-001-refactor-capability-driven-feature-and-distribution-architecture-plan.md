...
The canonical Mermaid language surface will therefore become invariant. Every build that parses Mermaid will recognize all 35 admitted Mermaid 11.16 families and expose the same canonical semantics, source spans, and family vocabulary. Optional analysis and editor products project that vocabulary for all families without reparsing. Cargo features will select only observable product APIs, outputs, heavy layout or math engines, system adapters, and tool-only capabilities. The main Rust facade and its Rustdoc integration may expose the shared result-named `complete-svg` convenience aggregate; workflow, transport, and target-specific combinations remain direct artifact recipes, while runtime policy selects determinism, time, randomness, text measurement, and resource limits for each operation.
...
- R5. Keep the positive capability leaves intuitive and additive. Delete the old `preset-*` feature family and its capability-descriptor table; retain the result-named `complete-svg` convenience aggregate only on the user-facing `merman` facade and `merman-rustdoc` integration crate. Browser package names, the single Typst `publish` package profile, CLI release profiles, and native SDK profiles are owner-specific artifact identities with direct feature lists, not Cargo features. Deterministic output additionally requires the explicit deterministic runtime constructor.
- R6. Capability-bearing implementation and transport crates use empty defaults: `merman-core`, analysis, editor-core, ASCII, render/export, bindings-core, C FFI, UniFFI, WASM, Typst, layout helpers, and renderer helpers such as Roughr. The `merman` facade and `merman-rustdoc` integration crate may provide the `complete-svg` convenience default; the Rustdoc default mirrors the facade so accepted documentation examples render without extra feature study, while the CLI default expands its complete command leaves directly. Every release artifact nevertheless uses an explicit artifact recipe with `default-features = false`; no release claim is inferred from a crate default. A normal `merman` dependency must render complete native SVG without additional feature study.
...
- R23. Supersede ADR-0069's multi-artifact single-package decision with two required browser products: the full convenience package `@mermanjs/web` and the complete SVG-only package `@mermanjs/web-render`. Independently admitted workflow-specific slim candidates are `@mermanjs/web-analysis`, `@mermanjs/web-editor`, and `@mermanjs/web-ascii`; each is published only when it owns a direct workflow and clears R27 after the final dependency graph. Each retained package contains exactly one intended WASM artifact and its matching wrapper, declarations, manifest, legal material, and input provenance. Browser package names must not imply Node or SSR support.
...
- AE6. `npm pack --json` for every retained browser package lists exactly one `.wasm`. An admitted `@mermanjs/web-editor` cannot resolve renderer exports; `@mermanjs/web-render` contains complete SVG, Cytoscape, ELK, and math capability but no editor, analysis, or ASCII API. A workflow-specific candidate below the 15-percent threshold is absent from the release contract and its documented workflow uses `@mermanjs/web`. The full package contains one WASM, not a duplicate `./full` artifact.
...
**Decision:** Public leaves use output, engine, environment, or tool vocabulary. The only aggregate Cargo feature is the result-named `complete-svg`, exposed by the user-facing `merman` facade and `merman-rustdoc` integration crate; product recipes and package profiles remain owner-specific artifacts. Incidental dependencies remain hidden with `dep:` forwarding.
...
#### KTD4. Low-level defaults are empty; result-named product conveniences remain

**Decision:** Capability-bearing implementation and transport crates use empty defaults: core, analysis, editor-core, ASCII, render/export, bindings-core, FFI, UniFFI, WASM, Typst, layout helpers, and Roughr. The user-facing `merman` facade and `merman-rustdoc` integration crate default to the same Cargo convenience aggregate named `complete-svg`, which means `svg`, `layout-cytoscape`, `layout-elk`, and `math` and does not compile system adapters. The Rustdoc default keeps accepted examples usable without extra feature study. The CLI default expands its complete command leaves directly. No `preset-*` feature is public or required. LSP, native bindings, browser packages, Typst, and every release build use exact `default-features = false` artifact recipes with direct leaf features.

**Why:** Empty implementation defaults prevent accidental feature unification. A single aggregate name that states the resulting capability is useful for the main Rust facade and its documentation integration; workflow, transport, and target-specific combinations belong to owner-specific artifact recipes rather than a second global vocabulary. Removing the old preset lattice eliminates duplicate names, false absence semantics, and cross-surface feature forwarding.
...
The normative public Cargo vocabulary is the positive leaf set (`svg`, `analysis`, `editor`, `ascii`, `png`, `jpeg`, `pdf`, `layout-cytoscape`, `layout-elk`, `math`, `system-clock`, `system-timezone`, `system-random`, `system-timing`, `network-icons`, `parallel-markdown`, and `shell-completions`) plus the `complete-svg` convenience aggregate exposed by `merman` and `merman-rustdoc`. Artifact profiles record direct feature lists for each product; they are not Cargo presets and are never exposed as capability IDs. Browser package names and the Typst `publish` package profile are product/package identities, not Cargo feature names.

Final single-artifact evidence admits analysis, ASCII, and editor as workflow-specific slim packages. It also admits complete-SVG `web-render` as a required capability product: after math and dependency upgrades it measures 17,023,368 raw bytes, 13,543,476 stripped bytes, 4,379,667 gzip bytes, and 3,043,714 Brotli bytes versus full at 18,284,917, 14,392,820, 4,646,814, and 3,215,042 bytes. Its 5.90-percent stripped reduction is recorded but is not the workflow-specific slim threshold. The Playground uses full in both realms unless a measured split beats that realistic baseline across download, compile cache, initialization, peak memory, and failure isolation.
...
**Decision:** Always publish the full browser package and the complete SVG-only render package, each with one WASM. Build analysis, editor, and ASCII as lockstep candidates, retaining each workflow-specific package only if its final unpacked artifact is at least 15 percent smaller than full and it supports an independent workflow. Shared generation and release validation keep retained APIs aligned; the full package does not embed sibling artifacts. Candidate rejection is a descriptor and documentation decision, not a hidden subpath alias.
...
**Decision:** Delete the `preset-*` Cargo feature family, capability-descriptor preset table, generated preset constants, and preset-specific release/documentation vocabulary. Keep only the positive leaf capabilities and the result-named `complete-svg` convenience aggregate exposed by `merman` and `merman-rustdoc`. Every CLI, binding, Web, Typst, Rustdoc release artifact, and Node artifact selects direct leaf features in its owner-specific recipe. Product names such as `@mermanjs/web-editor`, Typst `publish`, and `cli-release` remain package/artifact identities, not Cargo features or capability IDs.

**Why:** The old lattice mixed user-facing ergonomics with maintainer build recipes and encouraged users to treat additive features as exclusions. A single result-named aggregate shared by the main Rust facade and its Rustdoc integration preserves the common paths; every other combination is already explicit in an artifact profile and gains no value from another alias.
...
| RV67 | The default Rust facade can compile only system adapters without SVG, while Rustdoc lacks layout engines needed by accepted examples | Matching `complete-svg` defaults for the facade and Rustdoc integration, plus an explicit Rustdoc artifact recipe whose examples match its reported engine capabilities |
...
- **Approach:** Turn `platforms/web` into a private workspace/build owner and generate the required full and complete SVG-only render packages plus three browser-named workflow-specific slim candidates: `@mermanjs/web-analysis`, `@mermanjs/web-editor`, and `@mermanjs/web-ascii`. Each wrapper binds one U13 artifact profile and one WASM; workflow-specific candidates enter the public release contract only after clearing the independent-workflow and 15-percent gates. Delete public `./core`, `./render`, `./render-only`, `./ascii`, `./editor`, and `./full` multi-artifact exports from `@mermanjs/web`. Implement prerelease staging and dist-tag promotion as idempotent reconciliation: record old/target tags, verify every exact version, update and probe each tag, and restore the prior set on failure. This plan tests the workflow with dry runs or an isolated local registry only; it never mutates the real npm registry. Keep Playground editor/render in separate realms only after comparing split artifacts with the realistic two-realm full baseline. Browser package documentation explicitly rejects Node/SSR use rather than suggesting a loader workaround.
- **Test scenarios:** Package file ownership, workflow-specific 15-percent size admission, complete-render capability admission, cross-version rejection, one-WASM invariant, absent sibling exports, custom `module_or_path` initialization, stale/corrupt cache retry, capability mismatch, partial publication and mid-promotion recovery, legal drift, Playground split/full download/compile/init/heap evidence, editor/render startup and failure isolation, and the current msfjarvis.dev loader-patch regression.
- **Verification:** Build/test/smoke every package, run `npm pack --json` per package, enforce packed/unpacked/file-count and post-U11a-U11c measured WASM regression budgets, run Playground unit/build/browser smoke, and verify release contracts/status probes. Full and complete-render each have one WASM; every retained workflow-specific slim package clears admission. Record and explain the final package-size delta rather than weakening correct behavior to meet the provisional 16 MB forecast. Registry operations stop at dry-run or isolated local-registry evidence.
...
- [ ] Dependency evidence: retain `serde-saphyr 1.0.0-rc.1` because its selected serializer
  ...
  stable 1.0 releases when available. Continue internal-only ICU/font-metric optimization research
  without creating public features or weakening parity.
...
Each `npm pack --json` result must contain exactly one WASM, exact legal/provenance inputs, the artifact-profile-selected wrapper, and no sibling surface artifact. After U11a-U11c, `@mermanjs/web` publishes its measured packed/unpacked delta from the current multi-artifact package; the 16 MB forecast is not an acceptance ceiling. Complete-render publishes its same-profile comparison, while each retained workflow-specific slim package must remain at least 15 percent smaller unpacked than measured full. Every artifact receives raw/gzip/brotli regression budgets from the final graph rather than a guessed percentage. Browser package tests explicitly prove that none claims a Node/SSR runtime.
