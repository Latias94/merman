...
- R27. Package and WASM budgets must measure the artifact a user installs after complete 35-family, ELK, Cytoscape, and math admission. `@mermanjs/web` must fall from about 47.4 MB unpacked to at most 16 MB and contain one WASM; the limit reflects the current 13.1 MB stripped RaTeX-enabled full artifact plus packaging headroom. Each slim package must contain one WASM, be at least 15 percent smaller unpacked than the full package or be folded into it, and receive a new raw/gzip/brotli baseline only after U11a-U11c are final.
...
- Every npm package contains one WASM and the full convenience package is at most 16 MB unpacked instead of the current 47.4 MB multi-artifact package.
...
- **Verification:** Build/test/smoke every package, run `npm pack --json` per package, enforce packed/unpacked/file-count and post-U11a-U11c WASM budgets, run Playground unit/build/browser smoke, and verify release contracts/status probes. The full package is at most 16 MB unpacked, every retained slim package clears admission, and no package contains a duplicate full artifact. Registry operations stop at dry-run or isolated local-registry evidence.
...
Each `npm pack --json` result must contain exactly one WASM, exact legal/provenance inputs, the descriptor-selected wrapper, and no sibling surface artifact. After U11a-U11c, `@mermanjs/web` must be at most 16 MB unpacked; each retained slim package must remain at least 15 percent smaller unpacked than full and receives measured raw/gzip/brotli budgets from the final graph rather than a guessed percentage.
