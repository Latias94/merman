...
- **R10.** Rich external output must use a distinct artifact kind. Pinned Mermaid and external diagram code execute in an opaque-origin `sandbox="allow-scripts"` realm built as a self-contained local artifact; the realm has no `allow-same-origin`, parent DOM, storage, navigation, or ambient network authority and may return only budgeted strings through an authenticated MessagePort. Canonical Merman and ordinary returned Mermaid SVG remain subject to strict `assertSafeSvgForDom` and inline insertion. A rich artifact is independently sanitized under explicit element/attribute/URL/style budgets, rendered in a second zero-permission scriptless sandboxed iframe with a local CSP, and never inserted into the parent DOM. Unsupported export formats are disabled explicitly rather than silently degraded.
...
**Why:** ZenUML's upstream output is a positioned XHTML application inside SVG. Sanitizing only after its generator ran in a same-origin realm would leave the parent page exposed to dependency compromise or a renderer exploit; admitting the output into the parent DOM would then expand the trust boundary again.
...
**Test scenarios:** advanced ZenUML first render, reused render, invalid source recovery, opaque-origin proof, parent/storage/network access denial, forged bootstrap/port messages, sanitizer bypass corpus, oversized artifact, external URL, script/event attribute, tidy-tree Mindmap, ELK `mergeEdges`, realm timeout and replacement.
