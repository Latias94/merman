...
- R11. ELK `SeparateChildren` hierarchy planning does not recursively clone the remaining full graph or rescan all nodes for every ancestor. Every source node belongs to exactly one direct hierarchy scope, every source edge has exactly one owning scope using the lowest-common-ancestor or an equivalent source-backed rule, and a parent frame references collapsed child layout results rather than copying descendant collections. A stable hierarchy index and explicit postorder plan preserve child order, graph scope, direction, labels, nested layout merge order, and source-backed output while bounding preparation to `O(V + E + H + C)` time and `O(V + E + C)` additional space, where `H` is the number of hierarchy nodes visited and `C` is the number of hierarchy-boundary edge sections that ELK must materialize. Counters distinguish unique indexed items, total scope materialization, boundary sections, and kernel input work; linear frame visits alone are insufficient evidence.
...
- Latency candidates use clean adjacent commits, prebuilt digested executables, stable balanced A/A on both sides, and fresh alternating AB/BA confirmation with at least eight pairs. Preregister the materiality contract that matches the public scenario: the default `>10% AND >50 us` reporting budget, the checked-in low-latency formula, or a documented interactive, throughput, or high-volume objective whose bounds dominate A/A noise. The 50-us default is not a universal rejection rule. Owner-local repeated-work, complexity, or memory repairs may land under separately registered structural or memory evidence with semantic parity and representative public non-regression; added caches, protocol machinery, retained state, or broad abstractions require stronger measured evidence proportional to their cost.
...
- Structural controls use at least eight balanced pairs. Compute `noise = max(abs(median(A/A paired deltas)), 1.4826 * MAD(A/A paired deltas))`; `noise > 3%` makes the timing lane inconclusive, not the exact structural invariant. Otherwise require both the paired median and its 95% bootstrap upper bound to stay below `min(10%, max(5%, 3 * noise))` and the unit's preregistered absolute cap. A small owner-local structural candidate may still land when its named work reduction is exact, semantic and artifact gates pass, no comparison pair indicates a material regression, and the receipt explicitly withholds a latency claim. New caches, retained state, protocol machinery, or broad abstractions do not receive this exception.
...
- [x] U1-U13 are completed in dependency order. Mandatory U4-U8 outcomes and the U10 target index carry their required accepted claim labels; inability to land any of them leaves the goal blocked. Every optional candidate has one or more accepted claim labels (`accepted-structural`, `accepted-latency`, `accepted-memory`, `accepted-artifact`), or is `rejected` / `deferred-blocked` with a genuine invariant/platform reason.
- [x] Every accepted latency claim has stable A/A, at least eight fresh balanced adjacent AB/BA pairs, passing absolute/relative thresholds, raw evidence, and complete semantic/resource/control gates.
- [x] Every accepted structural or memory claim names input variables, reachable old/new bounds, added space, exact counter/curve evidence, and a representative normal-path regression result; it does not claim latency without a separate pass.
- [x] Rich inline HTML planning no longer has per-segment all-run rescans or growing-vector clones, and opaque host measurer observability is preserved.
- [x] Architecture `numIter` and Flowchart backend work are fail-closed under the single render-owned operation meter through neutral lower-crate work-control hooks, with no reverse dependency, double charge, silent clamping, algorithm substitution, or unchecked overflow.
- [x] ELK hierarchy preparation is iterative, bounded, small-stack safe, and source/order equivalent.
- [x] Dugong edge-label proxy and self-edge dummy retirement use batch deletion with exact graph/order equivalence.
- [x] State Rough rendering retains no cache across operations and preserves seeded/unseeded semantics.
- [x] Pinned registries use process-local immutable `OnceLock + COW` only if current-base confirmation and isolation gates pass.
- [x] The WASM host candidate either deserializes each non-nullish callback result once with unchanged callback/protocol behavior and accepted evidence, or is removed with a rejection receipt.
- [x] CLI stage lookup scales linearly in transaction cardinality. The sealed-manifest/compact-frontier subcandidate either proves linear metadata bytes plus the complete recovery-authority table and durability/recovery/tamper/foreign-file invariants on Linux, macOS, and Windows, or is removed while the target index remains.
- [x] The semantic-token candidate either has no interval-local finalists allocation and preserves every public token/error/ABI contract, or is removed with a rejection receipt.
- [x] The independently gated diagnostics-only analysis projection and core parser-capture candidates either avoid discarded rich work while preserving complete diagnostics, recovery, cancellation, resource, full core parse, and RichFacts behavior, or are removed with rejection receipts.
- [x] Each qualified Sequence, Requirement, and Mindmap metric candidate is independently accepted or removed; no global text cache or failed family residue remains.
- [x] Previously rejected Flowchart index, parser no-facts, no-report, Resvg, PNG, and Requirement residual directions were not reopened.
- [x] One fixed-budget attribution pass covers native rendering, browser-WASM, CLI batch, and editor/analysis. At most two newly discovered owner-local candidates were admitted through R1-R5; every other remaining hotspot records its absolute cost, reachable upper bound, owner, priority, and queued/rejected disposition.
- [x] `S -> F` controls and common-row `R -> F` context are complete and correctly labeled; no broad comparison is used as candidate causality.
- [x] Every surviving public Rust API, resource/error, WASM/browser protocol, or CLI transaction-format break appears in a breaking inventory and the applicable upgrade guide/release projection; if no public break survives, the closure receipt states that explicitly.
- [x] `PERF_PLAN.md`, `BENCHMARKING.md`, `RUNBOOK.md`, dated receipts, code, tests, and claim wording agree.
- [x] Abandoned attempts, temporary probes, dormant switches, obsolete helpers, superseded cache/journal paths, and candidate-only abstractions are removed.
- [x] Formatting, Clippy, strict verification, SVG DOM parity, affected package tests, browser protocol tests, CLI recovery tests, performance gates, review, and `git diff --check` pass or have explicit external-tool limitations. The final dependency-only tail uses focused local gates and delegates the exact whole-repository rerun to PR CI per maintainer direction. External limitations cannot substitute for the Linux/macOS/Windows evidence required to accept compact CLI persistence; that optional candidate was removed rather than accepted.
- [x] Conventional Commits contain only intentional program files/hunks. Existing `rust_out/`, `test-results/`, and unrelated user changes remain untouched and uncommitted.
