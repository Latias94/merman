...
- R4. Caller-requested cancellation and source rejection must remain outcomes outside a ready generation, and caller-cancelled work must never become a diagnostic or a cache entry. A custom parser that returns cancellation to a non-cancellable facade is a parser contract violation and must surface as a protected internal diagnostic, never as a Mermaid parse diagnostic.
...
- AE3. Given caller-controlled cancellation inside a custom overlay or built-in family loop, the caller receives cancellation, no diagnostic is fabricated, and the generation cache remains unchanged. Given an overlay that returns cancellation to a non-cancellable facade, analysis reports a protected internal parser-contract diagnostic rather than a Mermaid parse diagnostic. Given two waiters sharing a job, cancelling one detaches it without failing the other; the underlying job is cancelled only after the last waiter leaves or the session cancels.
...
- **Test scenarios:**
  ...
  6. Caller-controlled cancellation before, during, and after family parsing returns cancellation and no ready generation; unexpected custom-parser cancellation through a non-cancellable facade produces only the protected internal contract-violation diagnostic.
