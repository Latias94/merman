...
- **Test scenarios:**
  ...
  3. Runtime policy, source descriptor, and max-source budget are frozen in the capture request and inspectable through read-only generation metadata.
...
- **Test scenarios:**
  ...
  11. One multi-fence Markdown document preserves each fence's stable id and host range across generation facts, first and reprojected diagnostics, editor queries, and LSP range conversion.
...
- U1 is done when `AnalysisGeneration` and exact snapshot policy are the sole rich Rust analysis ownership model and old names are deleted.
