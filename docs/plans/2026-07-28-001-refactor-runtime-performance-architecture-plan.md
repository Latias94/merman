...
- R6. Every candidate preregisters the materiality contract that matches its public scenario. The default latency screening budget is a greater-than-10-percent and greater-than-50-microsecond normalized end-to-end improvement on the same public operation and input, with both one-sided confirmation lower bounds clearing their thresholds; it is not a universal admission or rejection rule. Complete low-latency operations may use the checked-in noise-derived formula, and documented interactive, throughput, high-volume, memory, or structural work may use scenario-specific bounds that dominate A/A noise. For Node binding latency work, the public operation is the installed revision-owned product's asynchronous `createNodeEngine().executeOperation()` surface under independently calibrated cold and reused-engine schedules; direct `NativeEngine`/`WasmEngine` timing is owner attribution only. A small owner-local repeated-work, complexity, or memory repair may be accepted without a latency label when its exact structural evidence, semantic parity, artifact controls, and representative public non-regression pass. New caches, protocol machinery, persistent state, or broad abstractions require stronger measured benefit proportional to their cost. The current 16.7 ms Flowchart value is an experimental 60 Hz reference, not a product SLO or standalone admission gate. Before it can become an interactive contract, a receipt must freeze the production call-chain evidence, generated input, host state, warm/cold contract, logical operation, and one-sided 95% upper bound over candidate absolute latency.
- R7. Stage and microbench results locate a cause but do not independently admit a latency claim; fixed-repeat stress estimates are normalized by logical operation and are trend/throughput evidence only. A candidate that misses its applicable preregistered contract is reverted or never applied, and its durable receipt records whether it was rejected or inconclusive.
...
- The default latency screening budget is end-to-end `>10% AND >50 us`, not a universal engineering cutoff. A complete public operation with a frozen baseline below 500 us may preregister the noise-adaptive low-latency formula from `BENCHMARKING.md`; documented interactive, throughput, high-volume, memory, or structural work uses the matching scenario contract required by R6. Flowchart's 16.7 ms value remains a reference only until a receipt freezes the complete production call-chain, input, host, lifecycle, statistic, confidence-bound, and workload-budget evidence.
...
- **Outcome (2026-07-29):** The indexed adapter candidate was rejected and fully removed after public-operation calibration could not support admission. Its low/high-cluster memory lanes, contracts, generators, and dedicated tests were also retired. The independent Dugong batch-retirement change remains as a structural complexity repair from worst-case `O(T * (V + E))` repeated adjacency reconstruction to `O(T + V + E)`; its preregistered latency and native-memory confirmation was not completed, so the program makes no measured speedup claim for that commit. The original Flowchart adapter and generic scale/stress coverage remain.
...
- **Outcome (2026-07-29):** Partially accepted on adversarial complexity, rejected on ordinary latency. Transparent duplicate-ID groups reduce reference dependency construction from `O(D * R)` to `O(D + R)`, and hash membership reduces expanded-attribute validation from `O(A^2)` to expected `O(A)`. A consuming one-worker PNG candidate saved only 0.78%, while a conservative single-reader validator regressed by 0.57%; both candidates and their temporary API, test, and one-fixture benchmark code were removed. See `docs/performance/resvg_pipeline_candidates_2026-07-29.md`.
...
- **Files (editable):** Integration/contract tests, benchmark/tooling/docs/workflow integration files, `docs/performance/PERF_PLAN.md`, `RUNBOOK.md`, `BENCHMARKING.md`, dated outcome reports, and the existing verification-knowledge hierarchy. Add a new durable learning only when the result is not already captured by those sources.
...
- **Outcome (2026-07-29):** Completed with program-scoped verification. Multi-agent review found and fixed an unregistered benchmark-list failure, then removed rejected U8 one-fixture lanes, rejected U4 memory lanes/contracts/generators/tests, duplicate Flowchart Criterion IDs, and a second allocator implementation. U10 added no permanent script or test. The active benchmark list verifies 279 benches in seven lane groups; performance contracts, native-memory contracts, and native-memory driver contracts pass 75, 48, and 20 tests respectively. Dugong/Graphlib pass 391 tests; bindings-core passes 156; merman-render passes 1,216 with one skipped; complete-SVG passes 139 with one skipped; Node passes 49. Clippy and targeted formatting pass for all affected packages. The workspace-wide formatting check reaches only an unrelated pre-existing import-order change in `crates/xtask/src/cmd/artifact_profiles.rs`, which this program leaves untouched. No unresolved P0/P1 review finding remains. Release-range data stays historical context; causal claims use the accepted candidates' adjacent evidence rather than an aggregate cross-evolution ratio.
...
- Normalize fixed-repeat estimates by their declared logical operation count. Apply one-sided 95% bounds against the preregistered scenario contract: the default `>10% AND >50 us` reporting budget, the noise-adaptive low-latency formula from `BENCHMARKING.md`, or a documented interactive, throughput, high-volume, memory, or structural objective whose bounds dominate the control-noise floor. Flowchart's 16.7 ms value is an experimental 60 Hz reference only unless U4 freezes such production SLO evidence.
...
- [x] U1-U12 are complete in dependency order; an evidence-gated unit may close without a production diff under KTD11 with either a durable rejection receipt or an inconclusive receipt plus an open retest trigger and fixed next budget.
- [x] The capability- and byte-matched alpha.3 comparison remains honest historical context; current optimization claims use decision-grade adjacent evidence and identify unavailable or semantically changed rows.
- [x] Every accepted latency optimization has stable same-binary A/A calibration, a power-derived minimum of eight balanced adjacent A/B pairs with one-sided confirmation bounds, and passing semantic, error, resource, security, host, and control gates proportional to its changed surface. Accepted structural repairs instead name reachable input variables, old/new complexity and added-space bounds, and carry no latency claim without separate timing admission. Unbounded caches, retained global state, or scale-sensitive allocation changes require six-point allocation/peak-memory evidence; small bounded operation-local state may use a structural bound plus representative artifact and throughput controls.
- [x] Flowchart either ships an ordered adapter whose membership/boundary/index construction and added space are `O(V+E+C)`, whose retained observable descendant and mutation terms are separately charged as `D` and `M`, and whose public confirmation/memory bounds pass, or retains the original adapter with a rejected/inconclusive receipt. The frozen workload reports 16.7 ms as a reference unless production SLO evidence was admitted.
- [x] Each qualified typed-parser family either ships a family-owned no-fact mode with identical editor output, or retains the original parser with a rejected/inconclusive receipt and no candidate residue.
- [x] Kanban either ships one private operation-scoped prepared artifact with no duplicate label preparation, or retains its original artifact with complete rejected/inconclusive evidence and no candidate residue.
- [x] Bindings either ship a typed private overlay whose version-only path borrows the base engine and whose real overlays preserve all ceilings/errors, or retain the original recursive merge with complete rejected/inconclusive evidence and no candidate residue.
- [x] Facade no-report terminals either skip only discarded report projection, or retain the original terminals with complete rejected/inconclusive evidence; report-returning APIs remain complete in either branch.
- [x] ResvgSafe either ships one terminal XML traversal with all expanded-name, escape-decoding, internal-failure, security/resource/error-precedence contracts, or retains the immutable two-pass baseline with complete rejected/inconclusive evidence; downstream rendering parse remains.
- [x] Requirement residuals are materially improved with proof, explicitly rejected with a complete profile, or marked inconclusive with a bounded retest contract; no unqualified candidate residue remains.
- [x] Completed Mindmap, Sequence, config-clone, and Requirement-label optimizations were not duplicated or weakened.
- [x] `PERF_PLAN.md`, `RUNBOOK.md`, `BENCHMARKING.md`, CI contracts, dated receipts, and the existing verification-knowledge hierarchy agree with the shipped implementation and measured non-implementation decisions.
- [x] Obsolete helpers, shims, misleading benchmark names, temporary probes, and alternate implementations owned by this program are deleted.
- [x] Formatting, Clippy, focused and full affected-package tests, Node tests, performance gates, diff review, and dirty-worktree staging audit are complete. The unrelated workspace-formatting exception is recorded in U10.
- [x] Only intentional program files/hunks are committed with focused Conventional Commits; unrelated user changes remain untouched and uncommitted by this work.
