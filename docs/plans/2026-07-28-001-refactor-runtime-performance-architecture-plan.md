...
- R21. Cargo build/test/bench work runs serially and reuses the normal target directory. Formal sampling begins only after both clean committed candidates are built with frozen recipes and executable digests; build time and artifact churn never enter runtime ratios, and unrelated dirty files are neither staged nor modified.
...
- The ordinary threshold is end-to-end `>10% AND >50 us`. Flowchart or any other high-volume workload may use a preregistered exception only when it supplies the explicit call-volume/throughput/CPU/memory contract and the complete production call-chain, input, host, lifecycle, statistic, and confidence-bound evidence required by R6; otherwise 16.7 ms remains a reference only.
...
- **Files:** `crates/merman/benches/pipeline.rs`, Markdown/many-card/routed-recording-measurer lanes and memory registration, `crates/merman-render/src/kanban.rs`, `crates/merman-render/src/family.rs`, `crates/merman-render/src/svg/parity/kanban/render.rs`, prepared-render/family tests, hostile-label fixtures, recording measurer tests, and the candidate receipt.
- **Approach:** First characterize conversion, sanitization, wrapping, measurement, public end-to-end cost, host calls, and six-point memory by label role. If qualified, change private `BuiltinFamilyArtifact::Kanban` to `FamilyPair<KanbanDiagramRenderModel, KanbanPreparedArtifact>`; the prepared type owns an unchanged layout projection plus section/card/title/ticket/assigned label plans, sanitized XHTML, styles, wrap choices, metrics, and stable associations. SVG emission consumes it without repeating work, reports reflect actual calls, and prepared storage cannot duplicate source/XHTML beyond the registered memory bound. Rejected/inconclusive results restore the original pair type.
- **Test scenarios:** No cards; many sections/cards; long wrapping; Markdown emphasis/code/links; HTML/entities; Unicode; strict/loose security; ticket/assigned/priority/icon combinations; missing parent; custom style/theme/look; recording and failing host measurers; prepared vector/node mismatch; repeated deterministic SVG; public layout JSON before/after.
- **Verification:** Kanban semantic/layout/SVG/Look/security goldens and prepared integration pass; host call traces contain each logical request once in order; accepted work clears the power-sized end-to-end confirmation and six-point memory bounds; parser/cross-family controls remain within noise. Rejected/inconclusive work proves the original artifact is restored.
...
- **Files:** `crates/merman-bindings-core/src/common.rs`, `engine.rs`, `operation.rs`, operation-owned render/analysis/ascii request modules, unit/contract tests, `platforms/node/benchmark/corpus.json`, `platforms/node/scripts/benchmark/worker.mjs`, report-contract tests, transport semantic fixture consumers where needed, private construction/allocation probes, memory registration, and the dated candidate receipt.
- **Approach:** First add canonical Node public-operation lanes for empty, version-only, and real overlays under cold and reused-engine lifecycles, plus warmed-JIT/controlled-GC metadata, six-point batch-size and base-engine-size curves, private merge/serialization/engine-construction counters, memory bounds, and an experiment-qualification receipt. Preserve the existing recursive JSON merge as a test-only differential oracle with explicit missing/null/value semantics: objects merge recursively, every non-object replaces, analysis/merman wrappers normalize exactly as today, and unknown/duplicate behavior remains parser-governed. If qualified, store validated typed base options/ceiling alongside the immutable engine. Parse request bytes once into a private request-overlay type that owns schema validation, forbidden runtime-policy detection, operation-scoped resource validation, and behavioral-delta classification. Borrow the base engine for empty/version-only overlays. For a real delta, merge typed values without JSON serialization and construct a short-lived bindings-private operation projection containing only the selected existing facade capability/config plus shared runtime/host measurer. Do not add a facade API for bindings. Keep operation and URI validation before request options exactly as characterized, and keep one-shot construction semantics separate from reusable-engine overlays.
...
- **Verification:** Bindings-core and transport contract suites pass; the differential oracle proves identical selected-operation options and complete errors; allocation/construction instrumentation proves no merged serialization or replacement engine for version-only. An accepted candidate clears the power-sized Node confirmation or a preregistered high-volume contract, with fixed cold/steady and warmed-JIT/controlled-GC lifecycles, six-point throughput/RSS curves, and bounded request-size versus base-engine-size growth. Rejected/inconclusive evidence removes the typed production overlay and retains the original merge path plus reusable lanes and the correct receipt/queue state.
- **Deletion:** For accepted code, remove `merge_request_options`, stored raw base JSON where no longer authoritative, repeated parse/serialize/reparse helpers, whole-engine request reconstruction, and the temporary oracle. For rejected/inconclusive code, remove the typed overlay and candidate-only helpers while retaining the original production merge.
...
| R14 Kanban | U5 | Closed prepared artifact, hostile labels, host measurer, memory curves, and Kanban A/B |
...
- [ ] Every accepted optimization has stable same-binary A/A calibration, a power-derived minimum of eight balanced adjacent A/B pairs with one-sided confirmation bounds, six-point allocation/peak-memory evidence, and passing semantic, error, resource, security, host, and control gates.
