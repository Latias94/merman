...
- AE2. Given `e` plus a combining accent, a ZWJ emoji such as a technologist, a flag, and CJK text, each survives wrap and paint as a complete grapheme, owns the correct continuation cells, and is not truncated into an invalid fragment.
...
- KTD3. **Breaking internal refactors are allowed; public changes are explicit.** Replace `char` cells, numeric message branching, duplicated relation documents, and special-case route paths when the replacement is demonstrably simpler and stronger. Public Rust/binding changes require migration tests and documentation. Governs R1-R9 and R14-R26.
...
| Preserve the `char` cell and patch combining/emoji cases | Avoids a central internal break | Cannot make measurement, truncation, style ownership, and emission agree for grapheme clusters | Rejected. |
...
| U10 | Deepen and modularize the shared relation graph | `relation_graph*`, shared geometry/document modules | U6 |
...
| U13 | Revalidate State on the new graph substrate | `state/*`, graph projection, state tests/docs | U4-U6 |
| U14 | Complete common XYChart semantics | `xychart/*`, core XYChart tests/docs | U2-U3 |
| U15 | Build shared terminal documents and field inventories for existing summary/full families | summary/full family modules and tests | U2-U3 |
...
| U28 | Deepen TreeView and conditionally upgrade Packet | `packet.rs`, `tree_view.rs`, tests | U15 disposition |
| U29 | Correct Mindmap/Journey and conditionally upgrade Kanban/Timeline | family modules and tests | U15 disposition |
...
- **Verification:** All 322 pinned-Mermaid-valid local discovery cases pass semantically; latest `mermaid-ascii` Mermaid-valid Sequence cases pass, while private extensions are listed separately rather than accepted accidentally.
...
- **Approach:** Create a field disposition matrix for classes, labels, annotations, members/methods, visibility/classifier/parameters/returns, generics, notes, namespaces, styles, links/callback metadata, relation kinds, independent source/target terminals, endpoint labels, and lollipop/interface forms. Replace the single-terminal adapter and rejection branch with independent terminals mapped to physical ports after direction planning. Reconstruct canonical member display text or validate the typed invariant when `display_text` is absent. Render meaningful fields in boxes/routes or structured fallbacks; accept harmless interaction metadata; return precise errors for unrepresentable semantic constructs. Extend group-aware routing across namespace/container boundaries where a readable route exists.
- **Test scenarios:** All 251 imported Class fixtures; `<-->`, `<..>`, `<|--|>`, `<|..|>`, `*--*`, `o--o`; TB/LR/BT/RL; namespace-local/cross-namespace relations; annotations; generic/multiline/CJK members; notes; endpoint cardinalities/labels; self/parallel/cyclic/dense topology; lollipop/interface nodes; aliases; styles; missing display-text typed models; and metadata-only fields.
...
- **Approach:** Inventory entity aliases, attributes, types/names, PK/FK/UK combinations, comments, relationship labels, both cardinalities including `MD_PARENT`, identifying/non-identifying kinds, direction/order, styles, and metadata. Preserve author insertion order in the typed model (migrating the current sorted map to an insertion-ordered representation while retaining compatibility JSON behavior). Add a width-aware attribute table with a compact exact fallback. Keep cardinality/identity projection family-owned and ensure routed and fallback forms disclose the same facts.
- **Test scenarios:** All 101 imported ER fixtures and latest reference ER corpus; `MD_PARENT`; declaration order and serialization round-trip; TB/LR/BT/RL; composite keys/comments; aliases; every cardinality pair; identifying kinds; self/parallel/reverse/cyclic/dense relations; attribute table width fallback; multiline/CJK/emoji labels; styles; hard budget errors; and unknown typed values.
...
### U28. Deepen TreeView and conditionally upgrade Packet
...
- **Approach:** TreeView already uses hierarchy as its primary encoding, so give it charset-specific connectors, folder/file/other node distinction, descriptions, empty-directory visibility, and an explicit icon policy through iterative validated traversal. Make Packet's current StructuredText range report exact first. Only if U15 admits Packet as Diagrammatic, add bounded proportional cells, row boundaries, range/ruler disclosure, label wrapping, and a compact exact fallback; otherwise defer packet geometry and keep the truthful report.
- **Test scenarios:** TreeView always covers files/folders/empty directories/descriptions/icons/deep trees/missing endpoints/duplicate ids, topology recovery at 80/100/120 columns, both charsets, safe graphemes, and deterministic fallback. Both Packet dispositions cover exact gaps/overlaps/row splits/wide ranges/long labels; the admitted branch additionally covers proportional cells, ruler/row structure, 80/100/120-column views, and compact fallback, while the rejected branch asserts StructuredText metadata and deferred grid evidence.
- **Verification:** TreeView exposes node type and description and retains Diagrammatic only after its R34 evidence passes. Packet ranges remain exact in both branches; an admitted Packet passes the spatial row/grid matrix, while a rejected Packet remains StructuredText without a geometry requirement.

### U29. Correct Mindmap/Journey and conditionally upgrade Kanban/Timeline
...
| Facade ASCII | `cargo nextest run -p merman --features ascii -j 1` | U1-U3, U17-U20, U22, U25 |
| Binding core ASCII | `cargo nextest run -p merman-bindings-core --features ascii -j 1` | U1-U3, U17-U20, U22, U25 |
| UniFFI/WASM ASCII | `cargo nextest run -p merman-uniffi --features ascii -j 1` and `cargo nextest run -p merman-wasm --features ascii -j 1` | U1-U3, U17-U20, U22, U25 |
| CLI ASCII | `cargo nextest run -p merman-cli --no-default-features --features ascii -j 1` | U1-U3, U17-U20, U22, U25 |
...
- **Flowchart gate:** Latest reference Mermaid-valid marker cases pass semantically; edge declaration permutations preserve rank/connectivity/marker ownership; all pinned exact fixtures are dispositioned.
- **Sequence gate:** All 322 pinned-Mermaid-valid discovery fixtures pass semantically; all latest reference Mermaid-valid fixtures pass; private extensions remain separately classified.
