...
| Group opacity | `BeginLayer`/`EndLayer` paint children as one composited group; Gantt axis ticks use this for their line and label. | Hosts must composite the group once. Multiplying each child's opacity is not equivalent when child paint overlaps. |
