...
- Active Mermaid parity focus: the repository is pinned to Mermaid 11.16.0, new 11.16 families are
  detector-visible, and `swimlane` plus `cynefin` now have source-backed semantic parsing/editor
  facts while render admission remains deferred until their typed layout/render paths are ported.
