...
- Completed in U7: updated `docs/lsp/CAPABILITIES.md` with the final ownership model, string Problems-code projection, analysis-preserved cardinality, and a parser-family diagnostic span matrix covering LALRPOP, XY Chart, Gantt, GitGraph, Timeline, C4, Architecture, and Kanban.
- Completed in U7: updated `docs/lsp/DIAGNOSTIC_PROTOCOL.md`, ADR 0070, `merman-core` and `merman-analysis` READMEs, and the VS Code README so docs no longer describe LSP as future work or preserve stale numeric-code, preview panel, pin, or workspace diagnostic claims.
...
- Final verification: `cargo fmt --all --check`; `git diff --check`; engineering memory validation; `cargo check -p merman-analysis -p merman-editor-core -p merman-lsp --tests`; `cargo test -p merman-analysis --lib parse_failure_deduplicates -- --nocapture`; `cargo test -p merman-analysis --lib fallback_recovery_merge_uses_structured_location_metadata -- --nocapture`; `cargo test -p merman-editor-core --test diagnostics`; `cargo test -p merman-lsp --test diagnostics`; `npm run check`; `npm test -- --test-reporter=spec`.
- Review note: `ce-code-review`'s standard flow was not invoked because this Codex turn's active developer instruction forbids spawning subagents unless the user explicitly asks for them. A local manual diff review covered the U6 compatibility cleanup and U7 docs against the plan; no actionable findings were found.

# Next Action

No required follow-up for this plan. Optional separate follow-up areas are Architecture/Kanban render-parser span coverage and unopened-file workspace diagnostics.
