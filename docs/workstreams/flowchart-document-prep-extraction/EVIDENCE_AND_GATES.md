# Flowchart Document Prep Extraction - Evidence And Gates
...
- `cargo nextest run -p merman-render flowchart` - PASS, 69 tests run, 69 passed, 138 skipped.
- `cargo run -p xtask -- compare-flowchart-svgs --check-dom --dom-mode parity --dom-decimals 3 --text-measurer vendored` - PASS.
- `cargo fmt -p merman-render` - PASS.
- `cargo fmt -p merman-render -- --check` - PASS.
- `cargo nextest run -p merman-render` - PASS, 207 tests run, 207 passed, 0 skipped.
