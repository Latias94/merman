...
| Performance evidence | `docs/workstreams/fearless-refactor/TYPED_MIGRATION_TIMING.md`, `docs/performance/spotcheck_2026-05-10_standard_canaries_stage_mmdr_toolchain.md`, `docs/performance/spotcheck_2026-05-14_flowchart_override_inventory_full_bench_gate.md`, `docs/performance/renderer_comparison_2026-05-10.md` | Covered |
...
- Class `parity-root` was rechecked after the earlier rendered-width pruning. Restoring
  `+handle(req: Request) : Response`, `+query(sql: String) : Rows`, and
  `+request() : Response` fixed `stress_class_styles_multiple_classdef_016` root max-width drift
  from `890.5px` back to the upstream `890.25px`; the affected Class layout golden was refreshed,
  and the no-growth text lookup budget is now `480`.
...
- `cargo nextest run -p merman-render --test class_svg_test`,
  ...
  `cargo nextest run -p merman-render --test layout_snapshots_test
  fixtures_match_layout_golden_snapshots_when_present` passed after removing the redundant Class
  `+quack()` and `+swim()` rendered-width overrides and refreshing the Duck no-attributes layout
  golden. `cargo run -p xtask -- report-overrides --check-no-growth` then passed with the text
  lookup budget tightened to `486`; the neighboring `test()` rendered-width override remains
  pinned because deleting it caused broad default-layout churn across 14 simple Class cypress
  fixture goldens.
- `cargo nextest run -p merman-render --test class_svg_test`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity-root --dom-decimals 3`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity --dom-decimals 3`, and
  `cargo nextest run -p merman-render --test layout_snapshots_test
  fixtures_match_layout_golden_snapshots_when_present` passed after removing the redundant Class
  `+template()` rendered-width override and refreshing the interfaces/abstracts layout golden.
  `cargo run -p xtask -- report-overrides --check-no-growth` then passed with the text lookup
  budget tightened to `485`.
- `cargo nextest run -p merman-render --test class_svg_test`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity-root --dom-decimals 3`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity --dom-decimals 3`, and
  `cargo nextest run -p merman-render --test layout_snapshots_test
  fixtures_match_layout_golden_snapshots_when_present` passed after removing the redundant Class
  `bar()` rendered-width override without layout golden drift. `cargo run -p xtask --
  report-overrides --check-no-growth` then passed with the text lookup budget tightened to `484`.
- `cargo nextest run -p merman-render --test class_svg_test`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity-root --dom-decimals 3`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity --dom-decimals 3`, and
  `cargo nextest run -p merman-render --test layout_snapshots_test
  fixtures_match_layout_golden_snapshots_when_present` passed after removing the redundant Class
  `+isOk() : bool` rendered-width override and refreshing
  `stress_class_dense_namespaces_generics_001.layout.golden.json`. `cargo run -p xtask --
  report-overrides --check-no-growth` then passed with the text lookup budget tightened to `483`.
- `cargo nextest run -p merman-render --test class_svg_test`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity-root --dom-decimals 3`,
  `cargo run -p xtask -- compare-class-svgs --check-dom --dom-mode parity --dom-decimals 3`, and
  `cargo nextest run -p merman-render --test layout_snapshots_test
  fixtures_match_layout_golden_snapshots_when_present` passed after removing the redundant Class
  `+logout()`, `+start()`, and `+addUser(user: User)` rendered-width overrides and refreshing the
  affected class layout goldens. A later `parity-root` recheck restored
  `+handle(req: Request) : Response`, `+query(sql: String) : Rows`, and
  `+request() : Response`, leaving the text lookup budget at `480`.
- `cargo fmt --check`, `cargo clippy -p manatee --all-targets --all-features -- -D warnings`,
...
- The follow-up Class `owns` cleanup removed one relation-label rendered width override,
  refreshed the affected association/aggregation/composition layout golden, and tightened the text
  lookup no-growth budget again to `498`.
- The Class `manages` rendered width override remains retained because deleting it breaks
  `class_svg_namespaces_and_relation_labels_keep_upstream_geometry` on the `Company.Project`
  cluster geometry.
- The follow-up Class `may-fail` cleanup removed one relation-label rendered width override,
  refreshed the affected dense-namespaces layout golden, and tightened the text lookup no-growth
  budget again to `497`.
