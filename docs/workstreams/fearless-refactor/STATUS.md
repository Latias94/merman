...
- Root viewport override no-growth is tightened to `378` after the GitGraph seeded auto-id
  ...
  `activation_explicit`, stacked-activation, and `arrows_variants` message-width fact
  corrections.
...
- root viewport: `flowchart` 95, `sequence` 75, `mindmap` 39, `c4` 35,
  `state` 34
