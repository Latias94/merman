...
- Root viewport override no-growth is tightened to `373` after the GitGraph seeded auto-id
  ...
  `activation_explicit`, stacked-activation, `arrows_variants`, and simple Cypress sequence
  message-width fact corrections plus the package sequence message/actor-width fact correction.
...
- root viewport: `flowchart` 95, `sequence` 70, `mindmap` 39, `c4` 35,
  `state` 34
