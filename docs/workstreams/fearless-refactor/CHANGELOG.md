...
- Removed sixteen additional low-width ER no-attribute calc-text-width lookups after both ER DOM
  parity modes stayed green, reducing ER text lookups from `91` to `75` and the global text lookup
  budget to `621`.
