...
reducing the text lookup total to 547. Later Class passes removed 34 lookup entries across the
...
`Cart`, `Server` rendered-width, `Dog`, and `Mineral` calc cleanups, reducing the text lookup total
to 513.
...
Total lookup entries reported by `xtask`: `513`.

| file | lookup entries |
| --- | ---: |
| `block_text_overrides_11_12_2.rs` | 123 |
| `class_text_overrides_11_12_2.rs` | 310 |
...
`Cart`, `Server` rendered-width, `Dog`, and `Mineral` calc cleanup passes refreshed the affected
layout golden as needed and reduced the Class text lookup total to 310 and the global text lookup
total to 513.
