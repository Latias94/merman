...
Gantt then dropped the generic `A`, `B`, and `C` task-width overrides after the font-metric
fallback proved stable, reducing text lookup debt by three more entries. C4 then moved its 17
type-line `textLength` pins into the C4 owner module and deleted the generated
...
Total lookup entries reported by `xtask`: `877`.
...
| `gantt_text_overrides_11_12_2.rs` | 41 |
