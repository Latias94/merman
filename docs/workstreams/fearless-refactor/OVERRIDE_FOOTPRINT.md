...
`connects`, `builds`, `parses`, `emits`, `feedback`, and `returns` cleanups, reducing the text
lookup total to 502.
...
Total lookup entries reported by `xtask`: `502`.

| file | lookup entries |
| --- | ---: |
| `block_text_overrides_11_12_2.rs` | 123 |
| `class_text_overrides_11_12_2.rs` | 299 |
...
`Cart`, `Server` rendered-width, `Dog`, `Mineral` calc, `Duck`, `Item`, `Order`, `Wheel`,
`connects`, `builds`, `parses`, `emits`, `feedback`, and `returns` cleanup passes refreshed the
affected layout goldens as needed and reduced the Class text lookup total to 299 and the global
text lookup total to 502.
