...
Requirement then dropped the paired `<<contains>>`, `<<satisfies>>`, and `<<traces>>` HTML
width/calc max-width lookups after both requirement parity modes stayed green without them.
...
Total lookup entries reported by `xtask`: `805`.
...
| `requirement_text_overrides_11_12_2.rs` | 120 |
