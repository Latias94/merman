...
`connects`, `builds`, `parses`, `emits`, and `feedback` cleanups, reducing the text lookup total
to 503.
...
Total lookup entries reported by `xtask`: `503`.

| file | lookup entries |
| --- | ---: |
| `block_text_overrides_11_12_2.rs` | 123 |
| `class_text_overrides_11_12_2.rs` | 300 |
...
`Cart`, `Server` rendered-width, `Dog`, `Mineral` calc, `Duck`, `Item`, `Order`, `Wheel`,
`connects`, `builds`, `parses`, `emits`, and `feedback` cleanup passes refreshed the affected
layout golden as needed and reduced the Class text lookup total to 300 and the global text lookup
total to 503.
