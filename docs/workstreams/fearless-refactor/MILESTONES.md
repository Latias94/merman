...
- Class text lookup debt dropped by 56 entries after the exact deterministic fallback pass, the
  ...
  `may-fail`, `references`, `int chimp`, `int gorilla`, `+int age`, `int id`, and `int[] id`
  cleanups, stayed green under Class DOM parity, layout snapshot, and strict gates, reducing the
  global text lookup total to 491.
