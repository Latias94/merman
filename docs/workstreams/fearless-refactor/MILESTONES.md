...
- Class text lookup debt dropped by 26 entries after the exact deterministic fallback pass, the
  `uses` plain-label cleanup, the `OK` pair cleanup, and the `ApiClient` cleanup with dense
  layout golden refresh stayed green under Class DOM parity, layout snapshot, and strict gates,
  reducing the global text lookup total to 521.
