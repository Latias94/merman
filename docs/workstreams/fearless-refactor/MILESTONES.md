...
- Class text lookup debt dropped by 43 entries after the exact deterministic fallback pass, the
  ...
  `builds`, `parses`, and `emits` cleanups, stayed green under Class DOM parity, layout snapshot,
  and strict gates, reducing the global text lookup total to 504.
