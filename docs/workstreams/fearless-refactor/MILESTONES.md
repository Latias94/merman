...
- Class text lookup debt dropped by 28 entries after the exact deterministic fallback pass, the
  `uses` plain-label cleanup, the `OK` pair cleanup, the `ApiClient` cleanup with dense layout
  golden refresh, and the later `ERROR` pair cleanup stayed green under Class DOM parity, layout
  snapshot, and strict gates, reducing the global text lookup total to 519.
