...
- Class text lookup debt dropped by 46 entries after the exact deterministic fallback pass, the
  ...
  `builds`, `parses`, `emits`, `feedback`, `returns`, and `wraps` cleanups, stayed green under
  Class DOM parity, layout snapshot, and strict gates, reducing the global text lookup total to
  501.
