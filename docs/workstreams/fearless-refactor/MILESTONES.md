...
- Class text lookup debt dropped by 51 entries after the exact deterministic fallback pass, the
  ...
  `may-fail` and `references` cleanups, stayed green under Class DOM parity, layout snapshot, and
  strict gates, reducing the global text lookup total to 496.
