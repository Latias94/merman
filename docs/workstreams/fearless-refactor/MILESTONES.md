...
- Class text lookup debt dropped by 36 entries after the exact deterministic fallback pass, the
  ...
  `Mineral` calc cleanups, followed by the `Duck` width cleanup, stayed green under Class DOM
  parity, layout snapshot, and strict gates, reducing the global text lookup total to 511.
