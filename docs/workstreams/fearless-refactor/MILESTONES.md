...
- Class text lookup debt dropped by 48 entries after the exact deterministic fallback pass, the
  ...
  `builds`, `parses`, `emits`, `feedback`, `returns`, `wraps`, `reads`, and `depends` cleanups,
