...
- Class text lookup debt dropped by 42 entries after the exact deterministic fallback pass, the
  ...
  `builds` and `parses` cleanups, stayed green under Class DOM parity, layout snapshot, and
  strict gates, reducing the global text lookup total to 505.
