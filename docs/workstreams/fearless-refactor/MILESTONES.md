...
- Class text lookup debt dropped by 44 entries after the exact deterministic fallback pass, the
  ...
  `builds`, `parses`, `emits`, and `feedback` cleanups, stayed green under Class DOM parity,
  layout snapshot, and strict gates, reducing the global text lookup total to 503.
