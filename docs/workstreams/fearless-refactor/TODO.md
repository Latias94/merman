...
- [x] Trim the Class calc-text-width override table to only browser-sensitive guards.
  Evidence: `crates/merman-render/src/generated/class_text_overrides_11_12_2.rs` now keeps 281
  lookup entries, down from 344, after deleting 63 entries across the exact-match pass, the
  ...
  `int[] id`, `+eat()`, `+mate()`, `+run()`, `+quack()`, `+swim()`, `+template()`, and `bar()`
  rendered-width cleanups.
  Both Class DOM parity modes and the layout snapshot test stayed green, and `report-overrides`
  now reports `484` text lookup entries. The `bar()`, `E`,
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `484` text lookup entries), so the strict gate blocks this
