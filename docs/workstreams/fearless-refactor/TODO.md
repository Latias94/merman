...
- [x] Reduce the Flowchart text override module to only the root-viewport guards that shared text
  ...
  calc-text-width pruning below brings the current total to `498`.
- [x] Trim the Class calc-text-width override table to only browser-sensitive guards.
  Evidence: `crates/merman-render/src/generated/class_text_overrides_11_12_2.rs` now keeps 295
  lookup entries, down from 344, after deleting 49 entries across the exact-match pass, the
  ...
  now reports `498` text lookup entries. The `bar()`, `E`,
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `498` text lookup entries), so the strict gate blocks this
