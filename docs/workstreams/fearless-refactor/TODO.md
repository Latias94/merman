...
- [x] Add parse/render timing samples before and after each typed migration.
  ...
  Mindmap/Architecture canaries. Gantt also dropped the generic `A` task-width row from
  `gantt_text_overrides_11_12_2.rs` while `compare-gantt-svgs --check-dom --dom-mode parity
