...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  Evidence: 47 root viewport overrides have been removed so far: 19 `architecture` entries after
  ...
  bounds and 9 `timeline` entries now covered by deterministic root output. One additional
