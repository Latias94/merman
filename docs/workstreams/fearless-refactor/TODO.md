...
- [x] Trim the Class calc-text-width override table to only browser-sensitive guards.
  Evidence: `crates/merman-render/src/generated/class_text_overrides_11_12_2.rs` now keeps 280
  lookup entries, down from 344, after deleting 64 entries across the exact-match pass, the
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `483` text lookup entries), so the strict gate blocks this
