...
- [x] Reduce the Flowchart text override module to only the root-viewport guards that shared text
  ...
  them. `report-overrides` now reports `669` text lookup entries after counting block-wrapped
  ...
  rect-with-title, cluster-title, and node/note label pruning.
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `669` text lookup entries), so the strict gate blocks this
