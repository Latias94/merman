...
- [x] Reduce the Flowchart text override module to only the root-viewport guards that shared text
  ...
  them. `report-overrides` now reports `618` text lookup entries after counting block-wrapped
  ...
  later ER zero-valued calcTextWidth, single-letter entity-label, and low-width calcTextWidth
  pruning.
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `618` text lookup entries), so the strict gate blocks this
  ...
  under the same parity gates, reducing the global text lookup total to 618.
