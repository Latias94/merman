...
- [x] Reduce the Flowchart text override module to only the root-viewport guards that shared text
  ...
  them. `report-overrides` now reports `666` text lookup entries after counting block-wrapped
  ...
  rect-with-title, cluster-title, node/note label, and quoted edge-label pruning.
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `666` text lookup entries), so the strict gate blocks this
