...
- [x] Reduce the Flowchart text override module to only the root-viewport guards that shared text
  ...
  calc-text-width pruning below brings the current total to `517`.
- [x] Trim the Class calc-text-width override table to only browser-sensitive guards.
  Evidence: `crates/merman-render/src/generated/class_text_overrides_11_12_2.rs` now keeps 314
  lookup entries, down from 344, after deleting 30 entries across the exact-match pass, the
  later `uses` plain-label cleanup, the final `OK` pair cleanup, and the `ApiClient` cleanup with
  the dense layout golden refresh, followed by the `ERROR` and `Payment` pair cleanups. Both Class
  DOM parity modes and the layout snapshot test stayed green, and `report-overrides` now reports
  `517` text lookup entries. The `bar()`, `E`,
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `517` text lookup entries), so the strict gate blocks this
