...
- [x] Add parse/render timing samples before and after each typed migration.
  ...
  Mindmap/Architecture canaries. Gantt also deleted the generated
  `gantt_text_overrides_11_12_2.rs` task-width table entirely after pruning the generic, test,
  task, duration, small-fixture, and final broad-label rows while Gantt parity and the strict gate
  stayed green. Future
...
- [x] Reduce the Flowchart text override module to only the root-viewport guards that shared text
  ...
  calc-text-width pruning below brings the current total to `523`.
- [x] Trim the Class calc-text-width override table to only browser-sensitive guards.
  Evidence: `crates/merman-render/src/generated/class_text_overrides_11_12_2.rs` now keeps 320
  lookup entries, down from 344, after deleting 24 entries across the exact-match pass, the
  later `uses` plain-label cleanup, and the final `OK` pair cleanup. Both Class DOM parity modes
  and the layout snapshot test stayed green, and `report-overrides` now reports `523` text lookup
  entries. The `bar()`, `E`, `IService`, `+run() : Status`, `Client`, and `+start()` entries
  remain because focused SVG tests assert those Mermaid HTML `max-width` caps explicitly.
...
- [ ] Delete overrides made obsolete by typed model or measurement fixes.
  ...
  and `523` text lookup entries), so the strict gate blocks this
  ...
  global text lookup total to 674. A later State node/note label pass removed `Idle`, `Moving`,
  `LOG`, `ACT`, and `this is a short<br/>note` under the same parity modes, reducing the global text
  lookup total to 669. A quoted State edge-label pass then removed `New Data`, `Succeeded`, and
  ...
  A later ER no-attribute calcTextWidth pass removed seventeen zero-valued lookup entries under the
  same ER parity gates, leaving only the `drawRect` clamp guard in that branch and reducing the
  global text lookup total to 643. A follow-up ER pass removed the single-letter entity label
  width lookups `A` through `F` under the same parity gates, reducing the global text lookup total
  to 637. A later ER pass removed nineteen additional low-width no-attribute calcTextWidth lookups
  under the same parity gates, reducing the global text lookup total to 618. A later ER pass
  removed the short relation labels `has`, `owns`, and `uses` under the same parity gates, reducing
  the global text lookup total to 615. A later ER pass removed the low-width relation labels
  `contains`, `hasMany`, `leads to`, `owned by`, `parent`, `places`, and `relates` under the same
