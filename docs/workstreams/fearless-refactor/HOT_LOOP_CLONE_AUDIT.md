...
1. Flowchart self-loop helper edge construction still starts from cloned source edges. This is
   safe but can be tightened later with a helper constructor that clones only the retained fields.
2. Sequence block/message label assembly still has several `String` clones around block collection
   and label wrapping. Audit only after isolating DOM-order-sensitive paths.
3. Class namespace/relation lookup construction still clones ids heavily because graphlib-style
