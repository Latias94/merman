...
| Replace fixture-scoped overrides where practical | Code changes plus generated table deletion | Started: eleven State root pins, thirteen Mindmap root pins, and ten Sequence root pins removed |
...
- State: `34` entries.
- Mindmap: `39` entries.
- Sequence: `188` entries.
- Root viewport total: `726` entries.
...
three title/accessibility root pins, two residual default-title root pins, and three simple
...
The simple note-right cluster now derives from the existing Sequence note/message bounds without
new SVG metric rows.
