...
- Current Timeline root viewport overrides: `8` entries after deriving the empty Timeline root
  ...
  shared rule.
- Current ER root viewport overrides: `6` entries after deriving the simple frontmatter-title
  ...
  `ATLAS-TEAMS` entity-label browser widths into ER-owned HTML label metrics. The six 16px
...
- Current Flowchart root viewport overrides: `43` inventory entries covering `49` fixture keys
  ...
  The latest icon-font probe keeps `stress_flowchart_icons_subgraph_mixed_061` retained: one small
  fixture already needs mixed `fa-play`/`fa-stop`/`fa-layer-group`/`fa-boxes-stacked` advances, so a
  single shared inline-box constant would trade one retained pin for glyph-shaped debt.
