...
| Block | `block/styles.ts` plus shared `styles.ts` | Covered | Composite cluster fade colors are source-backed. Shared edge thickness/pattern classes now apply to current Block edge paths, so `themeVariables.strokeWidth` reaches visible ordinary Block edges. Continue fixing only rules that apply to emitted Block SVG. |
...
| Flowchart | `flowchart/styles.ts` plus shared `styles.ts` | Covered for current HPD-080 gaps | `strokeWidth` reaches node CSS, `.edgePath .path`, and the visible `.edge-thickness-normal` edge class that current Flowchart paths actually emit; `nodeTextColor || textColor` drives Flowchart labels. Larger Flowchart CSS parity should stay fixture/source driven because many rules are tied to emitted shape attributes and renderer-specific DOM. |
...
| State | `state/styles.js` | Covered with deferred rules | State node, cluster, transition, label, note, marker, start/end, special-state, and title rules are covered. Focused renderer coverage now proves that current rough-path DOM really consumes `stateBkg` / `stateBorder` / `strokeWidth`, `mainBkg` for choice, `specialStateColor`, `innerEndBackground` plus `background` for end-state internals, and note colors, instead of only emitting provider CSS tokens. Neo gradient/drop-shadow and dependency-marker rules remain deferred without emitted support. |
| Timeline | `timeline/styles.js` | Covered for current visible DOM, with disabled-rule caveat | Classic section fill/text/line colors, root colors, line wrappers, and event brightness rules are emitted for current Timeline DOM. Redux Timeline now uses the source-backed current DOM branch: visible node paths consume `mainBkg` / `nodeBorder` / `strokeWidth` / drop-shadow CSS, labels consume `nodeBorder` / `fontWeight`, `lineWrapper line` consumes `nodeBorder` / `strokeWidth`, and redux node geometry omits the classic divider line. Disabled node/text colors honor `tertiaryColor` and `clusterBorder`, but ordinary compact smoke sources emit no `class="disabled"` DOM, so disabled colors are provider coverage rather than public visible-signal coverage unless a fixture actually emits disabled nodes. Neo-only support selectors remain deferred when required attributes/defs are absent. |
