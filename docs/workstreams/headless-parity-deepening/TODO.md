...
- [ ] HPD-080 [owner=codex] [deps=HPD-030] [scope=crates/merman-render/src/svg/parity,fixtures,xtask compare commands]
  ...
  Treemap, Requirement, and Mindmap theme/readability gaps, including Mindmap XHTML `span` label
  colors and root theme colors. Continue by scanning supported diagrams for blank output, hidden
  labels, black blocks, lost theme colors, and other functional renderability failures before
