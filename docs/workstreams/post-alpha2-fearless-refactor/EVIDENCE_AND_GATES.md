...
- `git diff --check -- CHANGELOG.md crates/merman-wasm/README.md docs/workstreams/post-alpha2-fearless-refactor`
- `python scripts/release-version.py check --version 0.7.0-alpha.2`
- `git diff --check -- docs/release .github/workflows platforms CHANGELOG.md docs/workstreams/post-alpha2-fearless-refactor`
- `cargo fmt --all --check`
...
- `cargo nextest run -p xtask compare_all_failures`
- `cargo nextest run -p xtask compare_all_options`
- `cargo nextest run -p xtask compare_all_diagram_selection`
...
- 2026-06-08: PA2R-130 audited the high-level binding package docs and corrected stale user-facing `@merman/web` references to the actual npm package name `@mermanjs/web` in `CHANGELOG.md` and `crates/merman-wasm/README.md`. Public ABI and generated-binding API shape stayed unchanged. Non-historical release-facing package-name grep found no remaining `@merman/web` references outside workstream history.
- 2026-06-08: PA2R-140 completed a release-operator final sweep. Workflow input examples now use `0.7.0-alpha.2`, release docs include the `npm` GitHub environment, and PyPI/npm text reflects that `merman`, `@mermanjs/web`, and pub.dev `merman` already exist. Registry queries returned npm `@mermanjs/web` `0.7.0-alpha.2`, pub.dev `merman` `0.7.0-alpha.2`, and PyPI `merman` `0.7.0a2`. Stale alpha.1/manual-first-publish grep passed. Release-facing `@merman/web` grep passed.
