...
restoration, dialog scroll ownership, the infinite Preview canvas, SVG Bounds, full-screen export,
...
Preview/Bounds state without host geometry. One focused mobile WebKit smoke covers layout, tap
...
- Open Share with SVG Bounds both off and on. Confirm the workspace link restores only the render
  workspace, while an issue link restores the selected page, editor tab, Preview mode, and Bounds
  preference without adding host dimensions or camera coordinates.
