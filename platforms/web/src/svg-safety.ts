...
class SvgSafetyScanner {
  ...
  assertSafe(): void {
    ...
    assertOnlyIgnorableRootTail(this.source, this.rootClosedAt, "Merman rendered");
...
function assertSafeElementName(name: string, inForeignObject: boolean): void {
  if (inForeignObject && !FOREIGN_OBJECT_LABEL_ELEMENTS.has(name)) {
    throw new Error("Merman rendered SVG with unsupported foreignObject content.");
  }
...
function assertSafeAttributes(attributes: SvgAttribute[], inForeignObject: boolean): void {
  for (const attribute of attributes) {
    ...
    if (
      ...
      throw new Error("Merman rendered SVG with interactive foreignObject attributes.");
    }
  }
}
