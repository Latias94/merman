...
- Added immutable `MermanIconPack`, transactional reusable `MermanIconRegistry.fromPacks`, and `MermanEngineServices` for constructor-owned icon registries and optional text measurement. Reusable engines now expose retryable, idempotent `close()`.
