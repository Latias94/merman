...
The following DrawingList operation is currently unreleased and requires a default-profile AAR
built from current source:

```kotlin
val result = Merman.execute("drawing-list-json", source)
check(result.mediaType == "application/vnd.merman.drawing-list+json;version=1")
val drawingListJson = result.data.toString(Charsets.UTF_8)
```
