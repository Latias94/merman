...
- Flutter native builds now consume the shared size-oriented default native artifact recipe; Apple slices compile-check the complete public C headers, macOS uses one XCFramework for CocoaPods and SwiftPM, and the pub package carries the project license, source provenance, and exact third-party license texts.
