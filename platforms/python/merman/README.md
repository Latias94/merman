...
> **Alpha:** Python and native APIs may break before the stable release. The source tree currently targets direct UniFFI binding API `4`, which is independent from the C ABI and text-measurement protocol. Install the Python wheel and native library as one artifact rather than mixing releases. Each published PyPI artifact preserves the documentation for its own binding API version.
