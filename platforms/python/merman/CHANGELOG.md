...
- Replaced parser-backed document facts with schema 2. Other versions are rejected before body decoding; remove `fact_source: "text_scan"` and Flowchart-only rich graph handling, and consume generic parser-backed items with explicit unavailable bodies.
