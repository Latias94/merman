...
Build targets and closure-evidence targets are separate. A `host` recipe builds for the machine running the recipe, while its checked dependency closure is explicitly scoped to the `x86_64-unknown-linux-gnu` reference target. That reference proves only the recorded Linux closure; it is not a macOS/Windows closure union or a cross-platform absence claim. A `target-set` recipe instead records one closure for every descriptor-owned target.
