...
Build targets and closure-evidence targets are separate. A `host` recipe builds for the machine running the recipe, while its checked normal-dependency closure is explicitly scoped to the `x86_64-unknown-linux-gnu` reference target and excludes build and proc-macro edges. That reference proves only the recorded Linux runtime dependency surface; it is not a full clean-build cost metric, a macOS/Windows closure union, or a cross-platform absence claim. A `target-set` recipe instead records one closure for every descriptor-owned target.
