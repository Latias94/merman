...
Build targets and closure-evidence targets are separate. A `host` recipe builds for the machine running the recipe, while its checked normal-dependency closure is explicitly scoped to the `x86_64-unknown-linux-gnu` reference target and excludes build and proc-macro edges. Cargo still resolves the normal dependencies of a proc-macro root for the running host, so the exact host-profile fingerprint is enforced only when the verifier itself runs on that Linux reference host; other hosts retain the semantic required/forbidden checks and report the observed fingerprint as non-enforced evidence. That reference proves only the recorded Linux runtime dependency surface; it is not a full clean-build cost metric, a macOS/Windows closure union, or a cross-platform absence claim. A `target-set` recipe instead records and enforces one closure for every descriptor-owned target on every verifier host.
