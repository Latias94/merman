const ACTIVE_SVG_ELEMENT_PATTERN = /<\s*(script|iframe|object|embed|audio|video|canvas)\b/i;
...
const CSS_UNSAFE_URL_PATTERN =
  /\b(?:url|@import)\s*\(\s*(['"]?)\s*(?:javascript:|data\s*:\s*text\/html|(?:https?:)?\/\/)/i;
...
export function assertSafePreviewSvg(svg: string): void {
  ...
  if (EXTERNAL_RESOURCE_PATTERN.test(svg) || CSS_UNSAFE_URL_PATTERN.test(svg)) {
